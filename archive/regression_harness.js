...
function softmaxBlend(stake, ev) {
  ...
  return clamp(stake * 0.6 + sm * stake * 2 * 0.4, 0, 0.25);
}
// Penalty stacks
const penaltyProduct = (dd, xg, regime, lee) => dd * xg * regime * lee; // baseline (V1-E bug)
const penaltyMin = (dd, xg, regime, lee) =>
  clamp(Math.min(dd, Math.min(1.0, regime), lee) * xg, 0.1, 1.0); // B1 fix
...
function stakePath(cfg, bet) {
  ...
  const ddFinal =
    cfg.penalty === "min" ? penaltyMin(dd, xg, regime, lee) : penaltyProduct(dd, xg, regime, lee);
...
const RegressionHarness = {
  run: (bets, cfg, label) => {
    ...
    const rpsScore = (fc, outcome) => {
      ...
      const ps = p.reduce((a, c) => a + c, 0) || 1;
      const pf = p.map((v) => v / ps);
  ...
  passes: (base, cand, gate) => {
    const TOL = 0.02;
    switch (gate) {
      case "ROI_DD":
        return cand.roi >= base.roi * (1 - TOL) && cand.maxDrawdown <= base.maxDrawdown * (1 + TOL);
      case "BRIER":
        return cand.brier <= base.brier * (1 + TOL);
      case "RPS":
        return cand.rps != null && base.rps != null && cand.rps <= base.rps * (1 + TOL);
      case "LOGLOSS":
        return cand.logloss <= base.logloss * (1 + TOL);
      case "EV_VETO":
        return cand.roi - base.roi >= -TOL;
      default:
        return false;
    }
  },
  ...
  drawCalibration: (bets) => {
    ...
    for (const [lg, d] of Object.entries(byLeague)) {
      if (d.n < 10) continue;
      const predDraw = d.predSum / d.n,
        realDraw = d.realised / d.n;
      report[lg] = {
        n: d.n,
        predictedDrawRate: +predDraw.toFixed(4),
        realisedDrawRate: +realDraw.toFixed(4),
...
function makeLedger(seed = 1, N = 200) {
  ...
  const bets = [];
  for (let i = 0; i < N; i++) {
    const mp = 0.3 + rnd() * 0.45; // model prob 0.30–0.75
...
const baseline = RegressionHarness.run(
  ledger,
  { kelly: "baseline", softmax: true, penalty: "product" },
  "BASELINE (current engine)"
);

const armA = RegressionHarness.run(
  ledger,
  { kelly: "correct", softmax: true, penalty: "min" },
  "B0+B1 softmax-ON  min-stack"
);
const armB = RegressionHarness.run(
  ledger,
  { kelly: "correct", softmax: false, penalty: "min" },
  "B0+B1 softmax-OFF min-stack"
);

const _fmt = (r) =>
  `${r.label.padEnd(34)} | ROI ${(r.roi * 100).toFixed(2).padStart(6)}% | DD ${r.maxDrawdown.toFixed(4)} | avgStake ${r.avgStake.toFixed(4)} | Brier ${r.brier.toFixed(4)} | logloss ${r.logloss.toFixed(4)}`;
...
let _decision;
const candidates = [armA, armB].filter((_r, i) => (i === 0 ? aPass : bPass));
if (candidates.length === 0) {
  _decision = "ESCALATE — no correct-Kelly arm passes vs (buggy) baseline. Human review required.";
} else {
  candidates.sort((x, y) => y.roi - x.roi || x.maxDrawdown - y.maxDrawdown);
  const winner = candidates[0];
  _decision = `SHIP: ${winner.label}  (softmax ${winner.label.includes("OFF") ? "REMOVED" : "KEPT"})`;
}
