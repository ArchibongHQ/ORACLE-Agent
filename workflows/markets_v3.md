...
| `ORACLE_V3_BLEND_PRICING` | `on` | Wave-4-accuracy (v5.1 §5.3). `on`: market-anchored blend applied to **every** candidate (not just odds≥4.00) — gates/EV/confidence/ranking/Kelly-stake all evaluate on `pBlend` with rescaled `CLASS_GATE_BLEND` bars (S≥1.0/M≥1.5/L≥2.0pt; heightened ×1.30; Class X unreachable by construction (unless `ORACLE_V3_X_CARVEOUT`, default off — see its row)). Implausible-edge caps + noise floor stay on RAW edge (hard invariant: blending never resurrects a capped pick). `off` = byte-identical legacy raw-edge gating (rollback). Fixes fake soft-market edges (Highest Scoring Half). |
| `ORACLE_V3_PATTERNS` | `shadow` | patterns-engine Wave 2 Phase 2 (2026-07-16). `on`: the Wave-1 deterministic pattern/trend detector (`marketsV3/patterns.ts`) relaxes ONLY `CLASS_GATE_BLEND`'s `minAdjEdgeBlend` for a pattern-backed candidate, scaled by detector strength (0.3 floor, up to 50% relaxation at strength 1.0) — every other bar (EV%, max odds, raw caps, noise gate) stays full-strength, plus an explicit `ev>0` value floor on top. **The repo's SECOND deliberate gate relaxation** (after `ORACLE_V3_X_CARVEOUT` below). `shadow`: tags `patternRelaxed:"shadow_pass"` without changing outcomes; also drives the fill-to-39 `v3BestFallback` pool (a fixture's best +EV candidate that failed ONLY the class_edge bar) and the same-wave Under→Asian-Handicap pivot (`analyzeFixtureMarketsV3` unconditionally strips every goals_ou/team_total Under from `evMarkets`, flag-independent — never gated by this flag). `off` = byte-identical to pre-Wave-2. Implemented in `marketsV3/evGate.ts`/`analyzeFixtureMarkets.ts`/`batch/index.ts`; see `packages/engine/test/patternGate.test.ts`, `patternsIntegration.test.ts`, `underAhPivot.test.ts`. |
| `ORACLE_V3_X_CARVEOUT` | `off` | High-conviction Class X carve-out (owner mod, same-day follow-up to Wave-4-accuracy — not part of the v5.1 spec) — **the repo's first deliberate gate relaxation** (`ORACLE_V3_PATTERNS` above adds a second; every other flag in this table only raises bars). Blend pricing made Class X unreachable by construction (raw-space −5pt exotic penalty vs blend-space edges: max rawEdgeBlend=0.40×0.12=0.048, minus 0.05 ⇒ can never reach X's 0.02 blend floor). This flag re-evaluates ONLY the blend-edge floor, with the raw-space exotic penalty rescaled into blend units (`X_CARVEOUT_PENALTY_RESCALE=1/3`, the same raw→blend ratio as the class bars) — every other X bar stays at full strength: `maxOdds≤15`, `blendEV≥12%`, EV floor, raw absolute/relative caps + noise gate (untouched, evaluated first), heightened X-exclusion (untouched). Additionally requires data-quality conviction: confirmed real xG AND completeness≥0.8 (`X_CARVEOUT_MIN_COMPLETENESS=0.8`, ⇒ wModel≥0.37/0.40). Reachable window is deliberately narrow — shortish-odds exotics (~3.0–3.5) with near-cap raw edge and near-full data quality; at long odds the 30% relative raw cap keeps X unreachable regardless. `shadow`: tags would-pass assessments (`xCarveout:"shadow_pass"`, carried into `V3AssessmentStat` for slate reports) without changing outcomes — promote to `on` only on ledger evidence from shadow-tagged assessments, never a hand-flip. `on`: admits qualifiers at `confidence:"medium"` (floor band, never higher); admitted picks stake/rank on the rescaled carve-out edge (`adjustedEdgeCarveout` ≥ 0.02 by construction — the standard `adjustedEdgeBlend` is ≤ −0.002 for every X candidate and would zero-Kelly the pick; shadow rows carry the same field as a counterfactual). Implemented in `marketsV3/evGate.ts`; see `packages/engine/test/xCarveout.test.ts`. |
...
| `ORACLE_UNIFIED_SLATE` | `on` | (stateful-rolling-elephant Phase 2) Two-state, not tri-state (`"legacy"` or anything-else → `"on"`; `packages/runtime/src/env.ts:453`). `on`: the delivered slate IS the cross-fixture pattern-aware two-tier pool — Tier① QUALIFIED (gate survivors) + Tier② WATCHLIST filling to 39, each row's `confidence` sourced from `V3DeliveryCandidate.mp` (the engine's own model probability) ALWAYS, never an LLM's self-reported number (`apps/worker/src/dailyBatch.ts`'s `deliveryCandidateToPick`, `:68-91`). `legacy`: restores pre-Phase-2 `summarizeBatch` delivery byte-identically (escape hatch; `packages/runtime/src/marketsV3/slateOutputs.ts:418`, `buildManifestDeliveredSlate` returns `undefined`). Pricing math, Kelly, the +EV floor, and the Under ban are unchanged by this flag either way — it re-plumbs selection/rendering only. |
| `ORACLE_V62_PATTERNS` | `shadow` | (stateful-rolling-elephant Phase 2A/3) Tri-state (`parseTriState`, `packages/runtime/src/env.ts:458`) — governs the v6.2 pattern catalog everywhere OUTSIDE markets-v3 (legacy `ExecutionEngine`/`scanAllMarketsFallback` ranking, Phase 2A; Phase 3's trap-flag/overall-basis work builds on the same flag). Distinct from `ORACLE_V3_PATTERNS` above, which only governs the markets-v3 gate-relaxation path. `shadow` (actual shipped default — the Phase 3 plan text names `"on"` as the intended eventual default, but the live `parseTriState(env.ORACLE_V62_PATTERNS, "shadow")` call ships `"shadow"`; verified in code before documenting, not copied from the plan) computes pattern rankings for the legacy path but never reorders picks. `on` applies the ranking boost live. `off` disables the legacy-path pattern catalog entirely. |
