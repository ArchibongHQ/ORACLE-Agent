...
vi.mock('@oracle/storage', () => ({
  GBrainAdapter: class { close = mockClose; },
}));

vi.mock('@oracle/runtime', () => ({
  loadEnv: () => ({}),
  buildConfig: () => ({ geminiApiKey: '', claudeApiKey: '', bankroll: 1000, rankingMode: 'CONFIDENCE_WEIGHTED', oddsApiKey: 'k', footballDataApiKey: 'fd' }),
...
const fakeBatch = {
  date: '2026-06-05', rankingMode: 'CONFIDENCE_WEIGHTED', jobs: [], actionableCount: 0, errorCount: 0,
};
const fakeResult = { batch: fakeBatch, records: [], manifest: { runId: 'r1' }, reportHtml: '<html>', reportPath: '/tmp/r.html' };
...
describe('dispatch — runtime delegation', () => {
  it('run with fixtures → calls runAnalysis, closes storage', async () => {
    mockFetchToday.mockResolvedValue({ jobs: [{ home: 'A', away: 'B', league: 'L', kickoff: 'k' }], source: 'api' });
  ...
  it('--no-llm zeroes the LLM keys passed to runAnalysis', async () => {
    ...
    const [, deps] = mockRunAnalysis.mock.calls[0] as [unknown, { config: { claudeApiKey: string; geminiApiKey: string } }];
    expect(deps.config.claudeApiKey).toBe('');
    expect(deps.config.geminiApiKey).toBe('');
  });

  it('resolve delegates to resolveDay', async () => {
    mockResolveDay.mockResolvedValue({ date: '2026-06-04', candidates: 3, resolved: [1, 2], unmatched: [3] });
