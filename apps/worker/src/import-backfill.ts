...
async function main(): Promise<void> {
  ...
  if (analyses?.length) {
    ...
    console.log(`[import] Analysis records: +${analyses.length}`);
  } else {
    console.log(`[import] Analysis records: not found at ${analysisPath}`);
  }
  ...
  if (resolutions?.length) {
    ...
    console.log(`[import] Resolution records: +${resolutions.length}`);
  } else {
    console.log(`[import] Resolution records: not found at ${resPath}`);
  }

  await storage.close();
  console.log(`[import] Done → ${dbDir}`);
}
