...
const deps = { storage: {} as never, config: { oddsApiKey: 'k', rankingMode: 'CONFIDENCE_WEIGHTED' } as never };

beforeEach(() => {
  vi.clearAllMocks();
  mockRunAnalysis.mockResolvedValue({ reportHtml: '<html>REPORT</html>', reportPath: '/tmp/r.html', batch: {}, records: [], manifest: {} });
});
...
describe('POST /analyze', () => {
  it('empty body → 400', async () => {
    const r = await handleRequest('POST', '/analyze', '', 'application/x-www-form-urlencoded', deps);
    expect(r.status).toBe(400);
    expect(r.body).toMatch(/Nothing to analyse/);
  });

  it('unparseable query → 400', async () => {
    const r = await handleRequest('POST', '/analyze', 'query=oneword', 'application/x-www-form-urlencoded', deps);
    expect(r.status).toBe(400);
  ...
  it('query with no odds match → 200 notice', async () => {
    ...
    const r = await handleRequest('POST', '/analyze', 'query=Foo+vs+Bar', 'application/x-www-form-urlencoded', deps);
    expect(r.status).toBe(200);
  ...
  it('query found → analyses and returns report html', async () => {
    mockFetchByName.mockResolvedValue({ home: 'Arsenal', away: 'Chelsea', league: 'Premier League', kickoff: 'k' });
    const r = await handleRequest('POST', '/analyze', 'query=Arsenal+vs+Chelsea&league=Premier+League', 'application/x-www-form-urlencoded', deps);
  ...
  it('pasted list → parseFixtureList → analyses', async () => {
    ...
    const r = await handleRequest('POST', '/analyze', body, 'application/x-www-form-urlencoded', deps);
  ...
  it('accepts JSON body', async () => {
    ...
    const r = await handleRequest('POST', '/analyze', JSON.stringify({ query: 'A vs B' }), 'application/json', deps);
    expect(mockRunAnalysis).toHaveBeenCalledOnce();
    expect(r.status).toBe(200);
  });
});
