...
describe('LAYER 7: CalibrationEngine.calculate (T36-T42, T111)', () => {
  ...
  it('T40: bbnParams computed for Premier League', () => expect(metrics.bbnParams['Premier League']).toBeDefined());
  ...
  it('T111: dynamicRhoParams field present (NEW-07)', () => expect(metrics.dynamicRhoParams).toBeDefined());
});
...
describe('NEW-22: CLV Backtest (T152-T154)', () => {
  const resolvedForBacktest: BetRecord[] = [
    { status: 'resolved', outcome: 'win', clv: 0.04, predictedClv: 0.06, marketType: '1x2', mp: 0.55, odds: 2.0, stakeAmt: 100 },
    { status: 'resolved', outcome: 'win', clv: 0.03, predictedClv: 0.05, marketType: '1x2', mp: 0.55, odds: 2.0, stakeAmt: 100 },
    { status: 'resolved', outcome: 'loss', clv: 0.05, predictedClv: 0.07, marketType: '1x2', mp: 0.55, odds: 2.0, stakeAmt: 100 },
    { status: 'resolved', outcome: 'win', clv: 0.05, predictedClv: 0.05, marketType: 'AH',  mp: 0.55, odds: 2.0, stakeAmt: 100 },
    { status: 'resolved', outcome: 'win', clv: 0.06, predictedClv: 0.05, marketType: 'AH',  mp: 0.55, odds: 2.0, stakeAmt: 100 },
    { status: 'resolved', outcome: 'loss', clv: 0.07, predictedClv: 0.06, marketType: 'AH', mp: 0.55, odds: 2.0, stakeAmt: 100 },
  ];
...
describe('addBet / resolveBet round-trip (T259-T262)', () => {
  ...
  beforeAll(async () => {
    const result = await engine.addBet({
      odds: 2.10, mp: 0.50, stakeAmt: 50,
      league: 'Premier League', home: 'A', away: 'B',
    });
...
describe('CalibrationEngine RPS with forecast data', () => {
  it('rps is null when no fp data in bets', () => {
    const bets: BetRecord[] = [
      { status: 'resolved', outcome: 'win',  mp: 0.60, odds: 2.0, stakeAmt: 100, homeGoals: 2, awayGoals: 1 },
      { status: 'resolved', outcome: 'loss', mp: 0.55, odds: 1.90, stakeAmt: 80,  homeGoals: 1, awayGoals: 2 },
    ];
  ...
  it('rps is computed when fp + goals present', () => {
    const bets: BetRecord[] = [
      { status: 'resolved', outcome: 'win',  mp: 0.60, odds: 2.0, stakeAmt: 100,
        fp: { home: 0.60, draw: 0.25, away: 0.15 }, homeGoals: 2, awayGoals: 1, league: 'PL' },
      { status: 'resolved', outcome: 'loss', mp: 0.55, odds: 1.90, stakeAmt: 80,
        fp: { home: 0.55, draw: 0.28, away: 0.17 }, homeGoals: 1, awayGoals: 2, league: 'PL' },
      { status: 'resolved', outcome: 'win',  mp: 0.50, odds: 2.10, stakeAmt: 60,
        fp: { home: 0.50, draw: 0.30, away: 0.20 }, homeGoals: 2, awayGoals: 0, league: 'PL' },
    ];
    const metrics = engine.calculate(bets);
    if (metrics.rps !== null) {
...
describe('§8.3 hierarchical bbnParams', () => {
  const bets: BetRecord[] = [
    { status: 'resolved', outcome: 'win',  mp: 0.60, odds: 2.0, stakeAmt: 100,
      league: 'Premier League', homeGoals: 2, awayGoals: 1 },
    { status: 'resolved', outcome: 'loss', mp: 0.55, odds: 1.90, stakeAmt: 80,
      league: 'Premier League', homeGoals: 1, awayGoals: 2 },
    { status: 'resolved', outcome: 'win',  mp: 0.50, odds: 2.10, stakeAmt: 60,
      league: 'UnknownLeague', homeGoals: 1, awayGoals: 1 },
  ];
  const metrics = engine.calculate(bets);
  ...
  it('per-league calibFactor stored in leagueData._leagueCalibFactors', () => {
    ...
    const cf = (lcf as Record<string, { calibFactor: number; shrinkage: number; n: number }>)['Premier League'];
...
describe('§8.3/§8.5 significanceAcceptGate', () => {
  ...
  it('rejects when |delta| < effectSizeFloor', () => {
    const base = Array(N).fill(0.220);
    const cand = Array(N).fill(0.2199); // delta = -0.0001 < floor 0.002
    const result = significanceAcceptGate(base, cand, { minN: 10, effectSizeFloor: 0.002, nBootstrap: BOOT });
    expect(result.accept).toBe(false);
  ...
  it('accepts a reliable large improvement (delta well below 0)', () => {
    ...
    const base = Array(N).fill(0.220);
    const cand = Array(N).fill(0.200);
    const result = significanceAcceptGate(base, cand, { minN: 30, effectSizeFloor: 0.001, nBootstrap: BOOT });
    expect(result.accept).toBe(true);
    expect(result.delta).toBeCloseTo(-0.020, 4);
