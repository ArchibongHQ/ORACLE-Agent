...
describe("LAYER 1b: ZIP Model (T101-T103)", () => {
  ...
  it("T101: ZIP matrix sums to 1.0", () => {
    ...
    for (let i = 0; i < matZIP.length; i++)
      for (let j = 0; j < matZIP.length; j++) s += matZIP[i]?.[j] ?? 0;
    expect(Math.abs(s - 1.0)).toBeLessThan(0.001);
  });
...
describe("LAYER 2: Market Extraction (T14-T18)", () => {
  ...
  it("T14: 1X2 probs sum to 1.0", () =>
    expect(Math.abs(mkt.hw + mkt.dr + mkt.aw - 1.0)).toBeLessThan(0.001));
  it("T15: BTTS in [0,1]", () => {
    expect(mkt.btts).toBeGreaterThanOrEqual(0);
    expect(mkt.btts).toBeLessThanOrEqual(1);
  });
...
describe("LAYER 3: SoS Adjustment (T19-T20)", () => {
  it("T19: SoS adj increases xG vs weak defense", () =>
    expect(adjustXGForSoS(1.5, 1.0, 1.35)).toBeGreaterThan(1.5));
  it("T20: SoS adj decreases xG vs strong defense", () =>
    expect(adjustXGForSoS(1.5, 2.0, 1.35)).toBeLessThan(1.5));
});
...
describe("LAYER 9: Power Vig Removal (T53-T55, T112)", () => {
  ...
  it("T53: Vig-removed probs sum to ~1.0", () =>
    expect(Math.abs(vig.home + vig.draw + vig.away - 1.0)).toBeLessThan(0.01));
  ...
  it("T112: Sub-1.0 overround handled by safety net", () => {
    const sub = powerMethodVigRemoval(2.2, 3.8, 5.0);
...
describe("LAYER 11: Drawdown Penalty (T113-T115)", () => {
  ...
  it("T114: 10% drawdown = 0.75 penalty", () => expect(getDrawdownPenalty(0.1)).toBe(0.75));
  it("T115: 8% drawdown = 0.75 penalty (NEW-11)", () =>
    expect(getDrawdownPenalty(0.08)).toBe(0.75));
});
...
describe("LAYER 17: RLM Direction (T119-T121)", () => {
  it("T119: Popular team shortening = steam, not RLM", () => {
    const r = lstmMarketDecoderProxy(0.5, 2.0, 1.9, true);
...
describe("LAYER 20: NEW FEATURES (T137-T148)", () => {
  ...
  it("T143: Draw calibration reduces overpriced draws (NEW-19)", () => {
    expect(drawCalibrationFactor(0.3, 0.25)).toBeLessThanOrEqual(1.0);
  });
  ...
  it("T145: Lambda inconsistent flagged when divergence >5% (NEW-16)", () => {
    expect(checkLambdaInconsistency(1.5, 1.2, 0.2).inconsistent).toBe(true);
  });

  it("T146: Steam chaser veto: compression + edge<5% (NEW-18)", () =>
    expect(isSteamChaser(true, 0.03)).toBe(true));
  it("T147: No steam veto: compression + edge>=5% (NEW-18)", () =>
    expect(isSteamChaser(true, 0.08)).toBe(false));
  it("T148: No steam veto: no compression (NEW-18)", () =>
    expect(isSteamChaser(false, 0.03)).toBe(false));
});
...
describe("LAYER 21: v29 AUDIT CRITICAL (T160-T179)", () => {
  it("T160: Kelly with explicit modelProb returns positive stake (BUG-C02)", () => {
    expect(optimizedKelly(0.1, 2.0, 0.85, false, 1.0, 1.0, 1.0, 0.25, 0.55)).toBeGreaterThan(0);
  });
  it("T161: Kelly with derived modelProb returns positive stake (BUG-C02)", () => {
    expect(optimizedKelly(0.1, 2.0, 0.85, false, 1.0, 1.0, 1.0, 0.25, null)).toBeGreaterThan(0);
  });
  it("T162: Canonical Kelly q=1-modelProb gives different result than market-implied q (BUG-C02)", () => {
    const withMp = optimizedKelly(0.1, 2.0, 0.85, false, 1.0, 1.0, 1.0, 0.25, 0.55);
    const oldStyle = optimizedKelly(0.1, 2.0, 0.85, false, 1.0, 1.0, 1.0, 0.25, 0.5);
  ...
  it("T168: CLV returns edgeRetentionFraction (BUG-M09)", () => {
    expect(clvProjection(0.1, 6, "1x2", 1.0).edgeRetentionFraction).toBeDefined();
  });
  it("T169: CLV returns survivalProb sigmoid (BUG-M09)", () => {
    expect(clvProjection(0.1, 6, "1x2", 1.0).survivalProb).toBeDefined();
  });
  it("T170: CLV survivalProb is valid probability [0,1] (BUG-M09)", () => {
    const v = clvProjection(0.1, 6, "1x2", 1.0).survivalProb;
    expect(v).toBeGreaterThanOrEqual(0);
    expect(v).toBeLessThanOrEqual(1);
  });
  it("T171: CLV zero edge → low survivalProb (BUG-M09)", () => {
    expect(clvProjection(0, 6, "1x2", 1.0).survivalProb).toBeLessThanOrEqual(0.1);
  });
...
describe("HOTFIX HF-A: Gaussian + Benford (T276-T283)", () => {
  ...
  it("T279: benfordMAD(<50 values)=null", () =>
    expect(benfordMAD(Array.from({ length: 30 }, (_, i) => i + 1))).toBeNull());
  it("T280: benfordMAD(200 values) returns number≥0", () => {
    const v = Array.from({ length: 200 }, (_, i) => 10 ** ((i % 9) * 0.3 + 0.1));
    const r = benfordMAD(v);
    expect(typeof r).toBe("number");
    expect(r!).toBeGreaterThanOrEqual(0);
  });
  it("T281: secondDigitFreq(null)=null", () => expect(secondDigitFreq(null)).toBeNull());
  it("T282: secondDigitFreq(<20 values)=null", () =>
    expect(secondDigitFreq([1.5, 2.0])).toBeNull());
...
describe("BLOCK 17: RPS (T368-T371)", () => {
  it("T368: A1 RPS perfect forecast = 0", () =>
    expect(rankedProbabilityScore({ home: 1, draw: 0, away: 0 }, "home")).toBe(0));
  it("T369: A1 RPS worst case = 1", () =>
    expect(
      Math.abs(rankedProbabilityScore({ home: 1, draw: 0, away: 0 }, "away") - 1)
    ).toBeLessThan(1e-9));
...
describe("LAYER 22: v29 BUG FIXES (T187-T192)", () => {
  ...
  it("T188: Draw calibration within conservative bounds (NEW-29)", () => {
    expect(drawCalibrationFactor(0.22, 0.28)).toBeLessThanOrEqual(1.2);
  });

  it("T190: Non-trivial S03 test: strong RLM on popular team (BUG-L04)", () => {
    const r = lstmMarketDecoderProxy(0.5, 1.5, 1.9, true);
    expect(r.rlm).toBe(true);
  });
...
describe("§8.1 buildBivariateMatrix", () => {
  it("BM-1: matrix sums to 1 (within truncation tolerance)", () => {
    const mat = buildBivariateMatrix(1.5, 1.2, 0.1);
