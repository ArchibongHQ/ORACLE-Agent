...
import { decide, buildEligibleBets, validateSelection } from '../src/decision/index.js';
import type { DecisionContext } from '../src/decision/index.js';
import type { EVMarket, DecisionOutput, PickRef } from '../src/types.js';
...
const BASE_CTX: DecisionContext = {
  fixture: { home: 'Arsenal', away: 'Chelsea', league: 'Premier League', kickoff: '2026-06-05T15:00:00Z' },
  fp: { home: 0.45, draw: 0.28, away: 0.27 },
  lambdaH: 1.45, lambdaA: 1.20,
...
describe('decide — LLM path', () => {
  ...
  it('falls back to deterministic when callClaude throws and no Gemini key', async () => {
    ...
    const bet = makeMarket();
    const { decision } = await decide([bet], BASE_CTX, { claudeApiKey: 'test-key', geminiApiKey: '' });
  ...
  it('uses Gemini fallback when Claude key absent and geminiApiKey present', async () => {
    ...
    const bet = makeMarket();
    const { decision } = await decide([bet], BASE_CTX, { claudeApiKey: '', geminiApiKey: 'gemini-key' });
    expect(decision).toHaveProperty('primaryPick');
    expect(decision).toHaveProperty('confidence');
    vi.doUnmock('@oracle/llm');
  });
  ...
  it('falls back to Gemini when Claude throws, then returns Gemini decision', async () => {
    ...
    const bet = makeMarket();
    const { decision } = await decide([bet], BASE_CTX, { claudeApiKey: 'claude-key', geminiApiKey: 'gemini-key' });
    expect(decision).toHaveProperty('primaryPick');
    expect(decision).toHaveProperty('confidence');
    vi.doUnmock('@oracle/llm');
  });
...
describe('validateSelection', () => {
  const eligible = [
    ...
    makeMarket({ cat: '1x2', market: '1x2', label: 'Home Win', ev: 0.05, mp: 0.45, modelProb: 0.45 }),
  ];

  it('passes through a valid pick unchanged', () => {
    const pick: DecisionOutput = {
      primaryPick: { market: 'Goals O/U', side: 'Over 2.5', odds: 2.1 },
      confidence: 0.7, rationale: 'good edge', rejectedAndWhy: [],
    };
  ...
  it('always passes NO_BET through', () => {
    const pick: DecisionOutput = { primaryPick: 'NO_BET', confidence: 0, rationale: 'no edge', rejectedAndWhy: [] };
  ...
  it('rejects pick not in eligible set → falls back to deterministic', () => {
    const pick: DecisionOutput = {
      ...
      confidence: 0.6, rationale: '...', rejectedAndWhy: [],
    };
    const result = validateSelection(pick, eligible);
  ...
  it('forces NO_BET when ML filter blocked', () => {
    const pick: DecisionOutput = {
      primaryPick: { market: 'Goals O/U', side: 'Over 2.5', odds: 2.1 },
      confidence: 0.7, rationale: '...', rejectedAndWhy: [],
    };
