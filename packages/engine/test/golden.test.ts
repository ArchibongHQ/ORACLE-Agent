...
import {
  poissonPMF, buildMatrix, extractMarkets, bivariatePoisson,
  rankedProbabilityScore, dixonColesTau, optimizedKelly,
  skellamPMF, skellamProbs, skellamAHCover,
} from '@oracle/engine';
...
describe('golden: poissonPMF', () => {
  ...
  it('P(1 | λ=1.5) ≈ 0.33470', () => {
    expect(poissonPMF(1, 1.5)).toBeCloseTo(0.33470, 4);
  });
...
describe('golden: bivariatePoisson', () => {
  ...
  it('P(1,1) = e^{-(l1+l2+l3)} * (l1*l2 + l3) (ground truth)', () => {
    ...
    expect(bivariatePoisson(1, 1, l1, l2, l3)).toBeCloseTo(
      Math.exp(-(l1 + l2 + l3)) * (l1 * l2 + l3), 8,
    );
  });

  it('sum over 14x14 grid ≈ 1 (truncation < 0.01%)', () => {
    let s = 0;
    for (let x = 0; x < 14; x++) for (let y = 0; y < 14; y++) s += bivariatePoisson(x, y, 1.2, 1.0, 0.1);
...
describe('golden: optimizedKelly', () => {
  ...
  it('positive edge → stake in (0, 0.15] (safety cap)', () => {
    ...
    const stake = optimizedKelly(0.10, 2.0, 0.85, false);
    expect(stake).toBeGreaterThan(0);
    expect(stake).toBeLessThanOrEqual(0.15);
  });

  it('council penalty halves the stake', () => {
    const normal = optimizedKelly(0.10, 2.0, 0.85, false);
    const penalised = optimizedKelly(0.10, 2.0, 0.85, true);
