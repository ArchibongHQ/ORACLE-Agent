...
describe("runSwarm — guards", () => {
  it("returns null when swarm disabled", async () => {
    const r = await runSwarm(5, fixture, [mkMarket("Over 2.5")], {
      ...baseConfig,
      enableSwarm: false,
      kimiApiKey: "k",
    });
  ...
  it("returns null when zero workers requested", async () => {
    const r = await runSwarm(0, fixture, [mkMarket("Over 2.5")], {
      ...baseConfig,
      enableSwarm: true,
      kimiApiKey: "k",
    });
...
describe("runSwarm — aggregation", () => {
  it("confidence-weighted consensus + low divergence when workers agree", async () => {
    ...
    const r = await rs(5, fixture, [mkMarket("Over 2.5"), mkMarket("Under 2.5")], {
      ...baseConfig,
      enableSwarm: true,
      kimiApiKey: "k",
    });
  ...
  it("flags high divergence when workers split", async () => {
    ...
    const { runSwarm: rs } = await import("../src/swarm/index.js");
    const r = await rs(6, fixture, [mkMarket("Over 2.5"), mkMarket("Under 2.5")], {
      ...baseConfig,
      enableSwarm: true,
      kimiApiKey: "k",
    });
