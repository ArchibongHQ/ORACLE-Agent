...
import { RAGSystem, PostmortemRegistry, postmortemRegistry, ROOT_CAUSES } from '@oracle/engine';
...
describe('RAGSystem.createEmbedding', () => {
  ...
  it('all dims are finite', () => {
    ...
    expect(emb.every(v => isFinite(v) && !isNaN(v))).toBe(true);
  });
...
describe('RAGSystem.addToStore', () => {
  ...
  it('stored entry has valid 12-dim embedding', () => {
    ...
    expect(last.embedding.every(v => isFinite(v))).toBe(true);
  });
  ...
  it('returns at most k entries', async () => {
    ...
    for (let i = 0; i < 6; i++) {
      await rag.addToStore(
        { home: `TeamH${i}`, away: `TeamA${i}`, league: 'Bundesliga',
          bayesian_lH: 1.3 + i * 0.1, bayesian_lA: 1.1,
          fp: { home: 0.45, draw: 0.30, away: 0.25 },
          evMarkets: [{ cat: '1x2', ev: 0.08 }],
          mc: { varMultiplier: 0.85 }, mes: 0.88 },
        { bayesian_lH: 1.3, bayesian_lA: 1.1 },
      );
    }
    const result = rag.findSimilar({ bayesian_lH: 1.4, bayesian_lA: 1.1, league: 'Bundesliga', evMarkets: [{ cat: '1x2', ev: 0.08 }] }, 3);
...
describe('PostmortemRegistry.add', () => {
  ...
  it('add() returns true for valid rootCause', () => {
    const ok = reg.add({
      fixtureId: 'TEST_001', date: '2026-01-15',
      homeTeam: 'Alpha', awayTeam: 'Beta',
      marketPicked: 'Home Win', marketResult: 'loss',
      failureType: 'test', signalsThatFired: ['S01'],
      signalsThatShouldHaveFired: ['S14'], rootCause: 'XG_CEILING_BREACH',
    });
    expect(ok).toBe(true);
  ...
  it('add() returns false for invalid rootCause', () => {
    const ok = reg.add({
      fixtureId: 'TEST_002', date: '2026-01-16',
      homeTeam: 'X', awayTeam: 'Y',
      marketPicked: 'Over 2.5', marketResult: 'loss',
      failureType: 'test', signalsThatFired: [],
      signalsThatShouldHaveFired: [], rootCause: 'INVALID_ROOT' as never,
    });
...
describe('PostmortemRegistry.check', () => {
  ...
  beforeAll(() => {
    reg.add({
      fixtureId: 'CHK_001', date: '2026-02-10',
      homeTeam: 'Wolves', awayTeam: 'Burnley',
      marketPicked: 'BTTS Yes', marketResult: 'loss',
      failureType: 'SSSVO ignored', signalsThatFired: ['S01','S03'],
      signalsThatShouldHaveFired: ['S10'], rootCause: 'SSSVO_IGNORED',
    });
  });
  ...
  it('returns empty array when no match above threshold', () => {
    ...
    const result = reg.check({
      fixtureId: 'QUERY', date: '2025-05-01',
      homeTeam: 'X', awayTeam: 'Y',
  ...
  it('returns matching entry for near-identical query', () => {
    ...
    const result = reg.check({
      fixtureId: 'CHK_QUERY', date: '2026-02-10',
      homeTeam: 'Wolves', awayTeam: 'Burnley',
      marketPicked: 'BTTS Yes', marketResult: 'loss',
      failureType: 'SSSVO ignored', signalsThatFired: ['S01','S03'],
      signalsThatShouldHaveFired: ['S10'], rootCause: 'SSSVO_IGNORED',
  ...
  it('results are sorted descending by similarity', () => {
    reg.add({
      fixtureId: 'CHK_002', date: '2026-02-10',
      homeTeam: 'Wolves', awayTeam: 'Burnley',
      marketPicked: 'BTTS Yes', marketResult: 'loss',
      failureType: 'SSSVO variant', signalsThatFired: ['S01'],
      signalsThatShouldHaveFired: ['S10'], rootCause: 'SSSVO_IGNORED',
    });
    const result = reg.check({
      fixtureId: 'CHK_QUERY2', date: '2026-02-10',
      homeTeam: 'Wolves', awayTeam: 'Burnley',
      marketPicked: 'BTTS Yes', marketResult: 'loss',
      failureType: 'SSSVO ignored', signalsThatFired: ['S01','S03'],
      signalsThatShouldHaveFired: ['S10'], rootCause: 'SSSVO_IGNORED',
...
describe('PostmortemRegistry.reset', () => {
  it('reset clears all entries', () => {
    ...
    r.add({
      fixtureId: 'X', date: '2026-01-01',
      homeTeam: 'A', awayTeam: 'B',
      marketPicked: 'Home Win', marketResult: 'loss',
      failureType: 'test', signalsThatFired: [],
      signalsThatShouldHaveFired: [], rootCause: 'DRAW_SUPPRESSED',
    });
    r.reset();
