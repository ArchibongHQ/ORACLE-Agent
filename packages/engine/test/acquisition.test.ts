...
const baseState: RunState = {
  ...
  pipeline: { fixture: { home: 'Arsenal', away: 'Chelsea', league: 'Premier League', date: '2026-06-10T15:00:00Z' } },
};
...
describe('acquisition — no geminiApiKey', () => {
  it('run completes without calling Gemini', async () => {
    const spy = vi.fn();
    vi.doMock('@oracle/llm', () => ({ fetchGeminiWithCascade: spy, callClaude: vi.fn(), MODELS: {}, THINKING_LEVELS: {}, ACQUISITION_CASCADE: [] }));
...
describe('acquisition — no fixture names', () => {
  it('skips acquisition without home/away', async () => {
    ...
    const result = await ExecutionEngine.run(stateNoFixture, { storage, config: makeConfig('fake-key') });
...
describe('acquisition — import failure', () => {
  it('engine still produces valid result when llm module unavailable', async () => {
    vi.doMock('@oracle/llm', () => { throw new Error('module not found'); });

    const result = await ExecutionEngine.run(baseState, { storage, config: makeConfig('fake-key') });
...
describe('acquisition — T1 xG merge', () => {
  ...
  afterEach(() => { vi.doUnmock('@oracle/llm'); });
  ...
  it('merges xH/xA from T1 response into telemetry', async () => {
    ...
    vi.doMock('@oracle/llm', () => ({
      ...
      callClaude: vi.fn(),
      MODELS: {}, THINKING_LEVELS: {}, ACQUISITION_CASCADE: [],
    }));
...
describe('acquisition — T3 soft context', () => {
  ...
  it('stores isDerby and softContext from T3 on state.telemetry', async () => {
    ...
    vi.doMock('@oracle/llm', () => ({
      fetchGeminiWithCascade: vi.fn().mockImplementation(async () => {
        ...
        return JSON.stringify({
          ...
          motivationScore: 1.1,
          travelKm: 350,
          softContext: [{ kind: 'motivation', text: 'North London Derby', source: 'Gemini T3', observedAt: '2026-06-10T15:00:00Z' }],
        });
      }),
      callClaude: vi.fn(),
      MODELS: {}, THINKING_LEVELS: {}, ACQUISITION_CASCADE: [],
    ...
    const state: RunState = {
      telemetry: { hOdds: 1.85, dOdds: 3.4, aOdds: 4.5, piH: 1550, piA: 1450 },
      pipeline: { fixture: { home: 'Arsenal', away: 'Tottenham', league: 'Premier League', date: '2026-06-10T15:00:00Z' } },
    };
    await ExecutionEngine.run(state, { storage, config: makeConfig('key') });
