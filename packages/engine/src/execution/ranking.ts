...
export function applyRankingMode(
  evs: EVMarket[],
  mode: RankingMode = "CONFIDENCE_WEIGHTED"
): EVMarket[] {
  const sorted = [...evs];
  switch (mode) {
    case "MAX_PROBABILITY":
      return sorted.filter((e) => e.ev > 0).sort((a, b) => b.modelProb - a.modelProb);
    case "MAX_EV":
      return sorted.sort((a, b) => b.ev * b.varianceMod - a.ev * a.varianceMod);
    default:
      return sorted.sort(
        (a, b) => b.ev * b.modelProb * b.varianceMod - a.ev * a.modelProb * a.varianceMod
      );
  }
}
