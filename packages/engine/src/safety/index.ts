...
export interface SignalMap extends Record<string, number | string | undefined> {
  S01?: number; S02?: number; S03?: number; S04?: number; S05?: number;
  S06?: number; S07?: number; S08?: number; S09?: number; S10?: number;
  S11?: number; S12?: number; S13?: number; S14?: number;
  _survivorshipBiasWarning?: string; _impliedEvFlag?: string;
}

export interface MarketConvergenceResult {
  market: string; odds: number;
  signals: SignalMap; totalScore: number; softmaxProb: number;
  tier: ConvergenceTier; activeSignals: string[]; missedSignals: string[];
  apexReason: string; negativeEvAlert: string | null;
}
...
export interface MLSafetyResult {
  mlAllowed: boolean; safetyScore: number;
  filtersTotal: number; filtersPassed: number;
  confidence: string; reason: string | null;
  altMarkets: string[]; drawRisk: DrawRisk;
}

export interface DrawRisk {
  score: number; tier: 'EXTREME' | 'VERY_HIGH' | 'HIGH' | 'MODERATE' | 'LOW';
  drawAdjustment: number; mlBlocked: boolean;
}

export interface AntiSycophancyResult { agreed: boolean; objection?: string; }
...
export class ConvergenceScorer {
  ...
  scoreMarket(market: Record<string, unknown>, resData: Record<string, unknown>, ragSimilar: Array<Record<string, unknown>> = []): MarketConvergenceResult {
    const signals: SignalMap = {};
    const mp = (market['mp'] ?? market['modelProb'] ?? 0) as number;
    const ip = (market['ip'] ?? ((market['odds'] as number) > 1 ? 1 / (market['odds'] as number) : 0)) as number;
    ...
    const sharpCount = ((frozenPayload?.['sharp_consensus'] as Record<string, number> | undefined)?.['bookCount'] ?? 0) as number;
    const crowdRoundBias = (resData['crowdWisdom'] as Record<string, unknown> | undefined)?.['_crowdRoundingBias'] === true;
    ...
    signals.S05 = ((resData['clvProjection'] as Record<string, number> | undefined)?.['survivalProb'] ?? 0) > 0.70 ? 1 : 0;
    ...
    const advCritique = (debate?.['adversary'] as { critiques?: Array<Record<string, unknown>> } | undefined)?.critiques?.find((c) => c['id'] === marketId);
    const refVerdict  = (debate?.['referee']   as { verdicts?: Array<Record<string, unknown>> } | undefined)?.verdicts?.find((v) => v['id'] === marketId);
    signals.S08 = (advCritique?.['decision'] === 'ACCEPT' && String(refVerdict?.['verdict'] ?? '').includes('+EV')) ? 2 : 0;
    const calibFactor = ((resData['ledger'] as Record<string, unknown> | undefined)?.['metrics'] as Record<string, number> | undefined)?.['calibFactor'] ?? (resData['calibFactor'] as number | undefined) ?? 1.0;
    signals.S09 = calibFactor > 1.0 ? 1 : 0;
    const bestAnalogue = ragSimilar[0];
    const HIGH_PROFILE = new Set(['Premier League','Champions League','La Liga','Bundesliga','Serie A']);
    const top5Leagues = ragSimilar.slice(0,5).map(a => String(a['league'] ?? '')).filter(Boolean);
    const survivorshipBiased = top5Leagues.length >= 4 && top5Leagues.every(l => HIGH_PROFILE.has(l));
    signals.S10 = (bestAnalogue?.['similarity'] as number ?? 0) >= 0.80 && bestAnalogue?.['sameCategoryAsQuery'] && !survivorshipBiased ? 1 : 0;
    if (survivorshipBiased) signals._survivorshipBiasWarning = '[SURVIVORSHIP_BIAS_SAMPLE] RAG analogues drawn exclusively from high-profile leagues';
    ...
    const cwAligns = cwPayload && !cwPayload['_aborted'] && (cwPayload['confidenceScore'] as number ?? 0) > 0.6 &&
      (String(market['label'] ?? '').toLowerCase().includes(String(cwPayload['dominantOutcome'] ?? '').toLowerCase()));
    signals.S11 = cwAligns ? 1 : 0;
    const outcome = String(market['label'] ?? '').includes('Home') ? 'home' : String(market['label'] ?? '').includes('Away') ? 'away' : 'draw';
    const fpProb = ((resData['fp'] as Record<string, number> | undefined)?.[outcome] ?? 0);
    signals.S12 = (fpProb > ip && ((resData['mc'] as Record<string, number> | undefined)?.['varMultiplier'] ?? 0) > 0.8) ? 1 : 0;
    ...
    const negativeEvAlert = evExcess > 0.05 ? `[NEGATIVE_EV_ALERT] Implied ${(ip*100).toFixed(1)}% exceeds model ${(mp*100).toFixed(1)}% — HARD REJECT.` : null;
    ...
    const activeSignals  = Object.entries(signals).filter(([, v]) => v === 1 || (typeof v === 'number' && v > 0)).map(([k]) => k);
    ...
    const apexReason = heavy.length === 0
      ? activeSignals.slice(0,3).join(', ') + ' providing baseline confidence'
      : heavy.join(' + ') + ' driving primary edge';

    return { market: String(market['label'] ?? market['market'] ?? ''), odds: market['odds'] as number, signals, totalScore, softmaxProb, tier, activeSignals, missedSignals, apexReason, negativeEvAlert };
  }

  compute(resData: Record<string, unknown>, ragSimilar: Array<Record<string, unknown>> = []): ConvergenceResult {
    const candidates = [
      ...((resData['evMarkets'] as Array<Record<string,unknown>> | undefined) ?? []).filter(m => !m['veto'] && (m['ev'] as number) > 0),
    ];
    const noResult = (noConvergence: boolean): ConvergenceResult => ({
      ...
      noConvergence, runnerUp:null, skipList:[], dispersionWarning:null, negativeEvAlert:null,
    });
    ...
    const dispersionWarning = (apex.totalScore - runnerUpScore) <= 3 && scores.length > 1
      ? '[LOW_DISCRIMINATION] APEX margin ≤3 points over runner-up' : null;
    ...
    return { apex, scores, overallTier, deploymentGuide, noConvergence, runnerUp: scores[1] ?? null, skipList, dispersionWarning, negativeEvAlert: negEvAlert };
  }
}

// ── MLSafetyFilter ────────────────────────────────────────────────────────────

const HIGH_RELIABILITY = new Set(['bundesliga','eredivisie','scottish premiership','austrian bundesliga','portuguesa primeira liga','primeira liga','belgian pro league','champions league','europa league','fifa world cup']);
const HIGH_UPSET       = new Set(['serie a','la liga','ligue 1','championship','mls','major league soccer','liga mx','french ligue 1','spanish la liga','italian serie a','english championship']);

const LEAGUE_DRAW_RATES: Record<string, { drawRate: number; baseRho: number }> = {
  "Premier League":  { baseRho:-0.13, drawRate:0.245 },
  "La Liga":         { baseRho:-0.16, drawRate:0.280 },
  "Serie A":         { baseRho:-0.18, drawRate:0.295 },
  "Bundesliga":      { baseRho:-0.14, drawRate:0.220 },
  "Champions League":{ baseRho:-0.10, drawRate:0.235 },
  "FIFA World Cup":  { baseRho:-0.07, drawRate:0.229 },
  "Default":         { baseRho:-0.13, drawRate:0.250 },
};

export class MLSafetyFilter {
  evaluate(fetched: Record<string, unknown>, resData: Record<string, unknown>, telemetry: Record<string, unknown>): MLSafetyResult {
    ...
    if (favOdds < 1.30 || favOdds > 1.70) return this._buildResult(filters, false, 'HARD REJECT: odds outside range', resData, telemetry);

    const eloDiff   = Math.abs((stats['home_pi_rating'] ?? 1500) - (stats['away_pi_rating'] ?? 1500));
    filters.push({ id:'S2', name:'Team Strength Gap', pass: eloDiff >= 120, reason: `Elo diff: ${eloDiff.toFixed(0)}` });

    const homeWinRate = stats['home_win_rate'] ?? 0;
    const homeAdvOk   = favIsHome ? (homeWinRate >= 0.60 || eloDiff >= 150) : eloDiff >= 180;
    filters.push({ id:'S3', name:'Home Advantage', pass: homeAdvOk, reason: `Home win rate: ${(homeWinRate*100).toFixed(0)}%` });

    const favXG      = favIsHome ? (stats['home_xg'] ?? (resData['bayesian_lH'] as number ?? 0)) : (stats['away_xg'] ?? (resData['bayesian_lA'] as number ?? 0));
    const favGS      = favIsHome ? (stats['home_goals_per_match'] ?? 0) : (stats['away_goals_per_match'] ?? 0);
    const oppGC      = favIsHome ? (stats['away_goals_conceded'] ?? 0) : (stats['home_goals_conceded'] ?? 0);
    filters.push({ id:'S4', name:'Attacking Superiority', pass: favXG >= 1.7 && (favGS >= 1.6 || favXG >= 1.8), reason: `xG:${favXG.toFixed(2)}` });

    const favGC         = favIsHome ? (stats['home_goals_conceded'] ?? 0) : (stats['away_goals_conceded'] ?? 0);
    const cleanSheetPct = favIsHome ? (stats['home_clean_sheet_rate'] ?? 0) : (stats['away_clean_sheet_rate'] ?? 0);
    const defenceOk = (favGC <= 1.2 || eloDiff >= 200) && (cleanSheetPct >= 0.35 || eloDiff >= 180);
    filters.push({ id:'S5', name:'Defensive Stability', pass: defenceOk, reason: `GC:${favGC.toFixed(2)}` });

    const dogGS     = favIsHome ? (stats['away_goals_per_match'] ?? 0) : (stats['home_goals_per_match'] ?? 0);
    filters.push({ id:'S6', name:'Underdog Attack Limited', pass: dogGS <= 1.1 || eloDiff >= 200, reason: `Dog GS:${dogGS.toFixed(2)}` });

    const totalXG   = (resData['bayesian_lH'] as number ?? 0) + (resData['bayesian_lA'] as number ?? 0);
    const goalsEnvOk = totalXG >= 2.3 && totalXG <= 3.2;
    if (totalXG <= 2.1) {
      filters.push({ id:'S7', name:'Goals Environment', pass:false, reason:`xG ${totalXG.toFixed(2)} ≤ 2.1 (HARD REJECT)` });
      return this._buildResult(filters, false, 'HARD REJECT: low-scoring environment', resData, telemetry);
    }
    filters.push({ id:'S7', name:'Goals Environment 2.3–3.2', pass: goalsEnvOk, reason:`xG ${totalXG.toFixed(2)}` });

    const favRest = (favIsHome ? (telemetry['restH'] ?? 7) : (telemetry['restA'] ?? 7)) as number;
    filters.push({ id:'S8', name:'No Schedule Congestion', pass: favRest >= 5, reason: `Rest: ${favRest}d` });

    const motivScore = (telemetry['motivationScore'] as number ?? 1.0);
    filters.push({ id:'S9', name:'Motivation Present', pass: motivScore >= 0.9, reason: `Motiv: ${motivScore.toFixed(2)}` });

    const favVelocity = (favIsHome ? (resData['lmuHome'] as Record<string, number> | undefined)?.['velocity'] : (resData['lmuAway'] as Record<string, number> | undefined)?.['velocity']) ?? 0;
    filters.push({ id:'S10', name:'Market Movement', pass: favVelocity >= 0, reason: `Vel: ${favVelocity.toFixed(4)}` });

    const isDerby      = (telemetry['isDerby'] as boolean ?? false);
    const keyInjury    = (telemetry['injPenH'] as number ?? 0) > 0.25 || (telemetry['injPenA'] as number ?? 0) > 0.25;
    const badWeather   = (((fetched['weather'] as Record<string, number> | undefined)?.['wind_mph'] ?? 0) > 35) || (((fetched['weather'] as Record<string, number> | undefined)?.['rain_mm'] ?? 0) > 15);
    const newMgr       = (telemetry['newMgrH'] as boolean ?? false) || (telemetry['newMgrA'] as boolean ?? false);
    const redFlagOk    = !isDerby && !keyInjury && !badWeather && !newMgr;
    filters.push({ id:'S11', name:'No Red Flags', pass: redFlagOk, reason: [isDerby&&'Derby',keyInjury&&'KeyInjury',badWeather&&'Weather',newMgr&&'NewMgr'].filter(Boolean).join(',') || 'Clean' });
    if (!redFlagOk && (isDerby || keyInjury)) return this._buildResult(filters, false, `HARD REJECT: ${isDerby?'Derby':'Key injury'}`, resData, telemetry);

    const trapCount = [totalXG<=2.1, !favIsHome&&eloDiff<150, favRest<4, motivScore<0.8, isDerby].filter(Boolean).length;
    filters.push({ id:'S12', name:'No Favorite Trap', pass: trapCount === 0, reason: `${trapCount} traps` });

    const highRel   = HIGH_RELIABILITY.has(league);
    const highUpset = HIGH_UPSET.has(league);
    filters.push({ id:'S13', name:'High Reliability League', pass: highRel && !highUpset, reason: league });
    if (highUpset) return this._buildResult(filters, false, `HARD REJECT: high-upset league`, resData, telemetry);

    const sharpDelta = (resData['sharpDelta'] as number ?? 0);
    const sharpBooks = (resData['fetched'] as Record<string, unknown> | undefined)?.['odds'] as Record<string, unknown> | undefined;
    const sharpBookCount = (sharpBooks?.['sharp_consensus'] as Record<string, number> | undefined)?.['bookCount'] ?? 0;
    if (sharpDelta > 0.10 && sharpBookCount >= 2) {
      filters.push({ id:'S16', name:'Sharp Consensus', pass:false, reason:`Sharp fading (delta:${sharpDelta.toFixed(3)})` });
      return this._buildResult(filters, false, 'HARD REJECT: sharp books fading', resData, telemetry);
    }
    filters.push({ id:'S16', name:'Sharp Consensus', pass: sharpDelta <= 0.03 || sharpBookCount < 2, reason:`Delta:${sharpDelta.toFixed(3)}` });

    const calibFactor = ((resData['ledger'] as Record<string, unknown> | undefined)?.['metrics'] as Record<string, number> | undefined)?.['calibFactor'] ?? 1.0;
    filters.push({ id:'S17', name:'Model Calibration Gate', pass: calibFactor >= 0.85, reason:`CF:${calibFactor.toFixed(3)}` });
    if (calibFactor < 0.70) return this._buildResult(filters, false, 'HARD REJECT: severe miscalibration', resData, telemetry);

    return this._buildResult(filters, true, null, resData, telemetry);
  }

  private _buildResult(filters: Array<{ pass: boolean }>, eligible: boolean, hardRejectReason: string | null, resData: Record<string, unknown>, telemetry: Record<string, unknown>): MLSafetyResult {
    ...
    let mlAllowed       = eligible && pct >= 0.70;
    let confidence      = '';
    if (!eligible || hardRejectReason) { mlAllowed = false; confidence = 'HARD_REJECT'; }
    else if (pct >= 0.85) confidence = 'HIGH_CONFIDENCE';
    else if (pct >= 0.70) confidence = 'MODERATE_CONFIDENCE';
    else { mlAllowed = false; confidence = 'REJECTED'; }
    const drawRisk = this._computeDrawRisk(resData, telemetry);
    if (drawRisk.mlBlocked) mlAllowed = false;
    return { mlAllowed, safetyScore: filtersPassed, filtersTotal, filtersPassed, confidence, reason: hardRejectReason, altMarkets: [], drawRisk };
  }

  private _computeDrawRisk(resData: Record<string, unknown>, telemetry: Record<string, unknown>): DrawRisk {
    let score = 0;
    ...
    if (lambdaDiff < 0.10) score += 15; else if (lambdaDiff < 0.20) score += 8;
    const totalXG = lH + lA;
    if (totalXG < 1.6) score += 15; else if (totalXG < 2.0) score += 8; else if (totalXG < 2.4) score += 4;
    score += Math.min(5, Math.round(Math.max(0, (lp.drawRate - 0.24)) * 100));
    const windMph = (resData['fetched'] as Record<string, unknown> | undefined)?.['weather'] as Record<string, number> | undefined;
...
export class AntiSycophancyCircuit {
  ...
  evFinderAgent(resData: Record<string, unknown>): Record<string, unknown> {
    ...
    for (const a of ((resData['analysis1x2'] as Array<Record<string, unknown>> | undefined) ?? [])) {
      ...
      const m = { ev: a['ev'] as number, mp: a['mp'] as number, ip: a['ip'] as number, odds: a['odds'] as number };
      ...
      proposed.push({
        ...
        odds: a['odds'], modelProb: m.mp, edge: m.ev,
  ...
  adversarialAgent(resData: Record<string, unknown>, finderOutput: Record<string, unknown>): Record<string, unknown> {
    ...
    const analyzeRisks = (bet: Record<string, unknown>) => {
      ...
      if ((resData['mc'] as Record<string, unknown> | undefined)?.['varFlag'] && edge < 0.1) {
        risks.push(`High variance environment (varFlag=true) with only ${(edge*100).toFixed(1)}% edge`);
        confidence -= 25;
      }
      ...
      if ((resData['drawdownPenalty'] as number ?? 1.0) < 1.0) {
        risks.push(`Drawdown penalty active — bankroll in protective mode (${((1 - (resData['drawdownPenalty'] as number))*100).toFixed(0)}% stake reduction)`);
        confidence -= 10;
      }
      ...
      if ((resData['mes'] as number ?? 1.0) < 0.85 && edge < 0.08) {
        risks.push(`MES veto zone: market efficiency ${((resData['mes'] as number ?? 0)*100).toFixed(1)}% — edge may be vig noise`);
        confidence -= 35; veto = true;
      }
      if (((resData['sensitivity'] as Record<string, number> | undefined)?.['fragilityScore'] ?? 0) > 6) {
        risks.push(`High fragility score (${((resData['sensitivity'] as Record<string, number>)['fragilityScore']).toFixed(1)}/10)`);
        confidence -= 20;
      }
      const upsetVeto = resData['upsetAlertVeto'] as string | undefined;
      if (upsetVeto && String(bet['label'] ?? '').toLowerCase().includes(upsetVeto)) {
        risks.push(`Upset alert triggered for ${upsetVeto} team`);
        confidence -= 25; veto = true;
      }
      if ((resData['ledger'] as Record<string, unknown> | undefined)?.['metrics'] &&
      ...
      const survivalProb = ((resData['clvProjection'] as Record<string, number> | undefined)?.['survivalProb'] ?? 1.0);
      if (survivalProb < 0.5) {
        risks.push(`CLV projection: edge survival probability only ${(survivalProb*100).toFixed(0)}%`);
        confidence -= 15;
      }
    ...
    for (const bet of ((finderOutput['proposed'] as Array<Record<string, unknown>> | undefined) ?? [])) {
      ...
      if (decision === 'DISPROVE') { totalScore += score; disprovedCount++; }
      critiques.push({
        id: bet['id'],
        market: bet['market'],
        originalScore: score,
        ...
        riskCalculation: decision === 'DISPROVE' ? `+${score} points (disproving claim)` : `Risk: -${score*2} if wrong dismissal`,
        decision,
  ...
  refereeAgent(
    ...
    for (const bet of ((finderOutput['proposed'] as Array<Record<string, unknown>> | undefined) ?? [])) {
      const critique = ((adversaryOutput['critiques'] as Array<Record<string, unknown>> | undefined) ?? [])
        .find(c => c['id'] === bet['id']) ?? { decision: 'ACCEPT', confidence: 100 };
      const adversaryDisproved = critique['decision'] === 'DISPROVE';
      const groundTruth = determineGroundTruth(bet, { veto: (critique['confidence'] as number ?? 100) < 50 });
      ...
      if (groundTruth && adversaryDisproved) {
        ...
        trigger = bet['confidenceBand'] === 'A' && (bet['edge'] as number ?? 0) > 0.1 ? 'GREEN' : 'YELLOW';
      } else {
        verdict = 'REJECTED'; trigger = 'RED';
      }

      // NEW-05: Force YELLOW if lineup unconfirmed < 3h to KO
      if (resData['lineupUnconfirmed'] && (resData['hoursToKO'] as number ?? 24) < 3 && trigger === 'GREEN') {
        trigger = 'YELLOW';
        verdict = verdict + ' [LINEUP_GATE]';
      }

      // B10-01: S14 [NEGATIVE_EV_ALERT] hard reject — absolute priority
      const convergenceNegEvAlert = (resData['convergence'] as Record<string, unknown> | undefined)?.['negativeEvAlert'] ?? null;
      const convergenceScores = (resData['convergence'] as Record<string, unknown> | undefined)?.['scores'];
      const betNegEvAlert = Array.isArray(convergenceScores)
        ? (convergenceScores as Array<Record<string, unknown>>).find(s =>
      ...
      if (trigger === 'RED' && (bet['edge'] as number ?? 0) > 0.08 &&
          critique['decision'] === 'DISPROVE' && (critique['confidence'] as number ?? 100) < 65 &&
          bet['confidenceBand'] !== 'E' && bet['confidenceBand'] !== 'D' &&
          ((resData['mes'] as number | undefined) ?? 1) > 0.75 &&
          !convergenceNegEvAlert && !betNegEvAlert) {
        trigger = 'YELLOW';
    ...
    const topBet = topBetVerdict
      ? { market: topBetVerdict['market'], odds: topBetVerdict['odds'], trigger: topBetVerdict['trigger'], edge: topBetVerdict['edge'] }
      : null;
    ...
    return {
      ...
      confirmedList: verdicts.filter(v => String(v['verdict'] ?? '').includes('+EV'))
  ...
  private _findAH(resData: Record<string, unknown>): string {
    const ah = ((resData['evMarkets'] as Array<Record<string, unknown>> | undefined) ?? [])
      .filter(m => m['cat'] === 'Asian Handicap' && !m['veto'] && (m['ev'] as number ?? 0) > 0.03);
  ...
  private _riskFlags(resData: Record<string, unknown>, referee: Record<string, unknown>): string[] {
    const flags: string[] = [];
    if ((resData['mc'] as Record<string, unknown> | undefined)?.['varFlag']) flags.push('[HIGH_VARIANCE] High variance environment');
    if ((resData['drawdownPenalty'] as number ?? 1.0) < 1.0) flags.push(`[DRAWDOWN] Drawdown penalty: ${((1-(resData['drawdownPenalty'] as number))*100).toFixed(0)}% stake reduction`);
    ...
    if (((resData['sensitivity'] as Record<string, number> | undefined)?.['fragilityScore'] ?? 0) > 6) flags.push('[HIGH_FRAGILITY] High sensitivity to input changes');
    if ((resData['ledger'] as Record<string, unknown> | undefined) &&
        ((resData['ledger'] as Record<string, unknown>)['metrics'] as Record<string, boolean> | undefined)?.['driftAlert']) flags.push('[DRIFT_ALERT] Recent calibration divergence');
    if (resData['lineupUnconfirmed'] && (resData['hoursToKO'] as number ?? 24) < 3) flags.push('[LINEUP_GATE] Starting XI unconfirmed < 3h to KO');
    if (resData['isArbitrage']) flags.push('[ARB_STATE] Market in arbitrage — pre-scaled for correct EV calc');
    if (((resData['clvProjection'] as Record<string, number> | undefined)?.['survivalProb'] ?? 1) < 0.5) flags.push('[CLV_FADE] Edge survival < 50% — close now if value exists');
    void referee;
  ...
  private _sovereignGap(resData: Record<string, unknown>): Record<string, unknown> | null {
    ...
    if (!top) return null;
    const gap = Math.abs(((top['mp'] as number | undefined) ?? 0) - ((top['ip'] as number | undefined) ?? 0));
