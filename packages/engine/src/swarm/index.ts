...
export function swarmWorkersForTier(tier: string): number {
  switch (tier) {
    case 'APEX':  return 7;
    case 'PRIME': return 5;
    case 'VIABLE': return 3;
    default:       return 0;  // MARGINAL / NOISE / UNKNOWN
  }
}
...
function buildWorkerPrompt(
  ...
  const bets = eligible.slice(0, 8).map((m, i) =>
...
function aggregate(votes: SwarmVote[]): { consensusPick: string; divergence: number } {
  ...
  for (const [pick, w] of weight) {
    if (w > bestWeight) { bestWeight = w; bestPick = pick; }
  }
...
export async function runSwarm(
  ...
  const votes: SwarmVote[] = settled
    .filter((r): r is PromiseFulfilledResult<SwarmVote> => r.status === 'fulfilled' && r.value != null)
    .map(r => r.value);
