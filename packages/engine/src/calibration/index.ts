...
export interface CalibrationMetrics {
  brier: number; recentBrier: number;
  rps: number | null; recentRPS: number | null;
  clv: number; roi: number;
  calibFactor: number; leagueData: Record<string, unknown>;
  ...
  driftAlert: boolean; resolvedCount: number; winRate: number;
  totalPnl: number; totalStaked: number;
...
export interface BetRecord {
  ...
  mp?: number; odds?: number; stakeAmt?: number;
  outcome?: string | null; clv?: number | null; qScore?: number;
  homeGoals?: number; awayGoals?: number; closingOdds?: number;
  expHomeG?: number; expAwayG?: number;
  fp?: Record<string, number>; marketType?: string; predictedClv?: number;
  loggedAt?: string; resolvedAt?: string;
  liquidityTag?: LiquidityTag; clvSourceQuality?: ClvSourceQuality;
}
...
const LEAGUE_PARAMS: Record<string, { homeAvg: number; awayAvg: number; baseRho: number; kFactor: number; avgGA: number; drawRate: number }> = {
  "Premier League":   { baseRho:-0.13, homeAvg:1.48, awayAvg:1.22, kFactor:15, avgGA:1.35, drawRate:0.245 },
  "La Liga":          { baseRho:-0.16, homeAvg:1.52, awayAvg:1.18, kFactor:12, avgGA:1.28, drawRate:0.280 },
  "Serie A":          { baseRho:-0.18, homeAvg:1.42, awayAvg:1.10, kFactor:12, avgGA:1.25, drawRate:0.295 },
  "Bundesliga":       { baseRho:-0.14, homeAvg:1.62, awayAvg:1.35, kFactor:10, avgGA:1.45, drawRate:0.220 },
  "Ligue 1":          { baseRho:-0.15, homeAvg:1.44, awayAvg:1.15, kFactor:10, avgGA:1.30, drawRate:0.260 },
  "Champions League": { baseRho:-0.10, homeAvg:1.55, awayAvg:1.25, kFactor:18, avgGA:1.40, drawRate:0.235 },
  "Europa League":    { baseRho:-0.12, homeAvg:1.50, awayAvg:1.20, kFactor:15, avgGA:1.35, drawRate:0.240 },
  "Eredivisie":       { baseRho:-0.12, homeAvg:1.72, awayAvg:1.38, kFactor:10, avgGA:1.52, drawRate:0.210 },
  "Championship":     { baseRho:-0.13, homeAvg:1.50, awayAvg:1.20, kFactor:8,  avgGA:1.35, drawRate:0.265 },
  "Default":          { baseRho:-0.13, homeAvg:1.45, awayAvg:1.15, kFactor:8,  avgGA:1.30, drawRate:0.250 },
};
...
const LEAGUE_TIER: Record<string, 1 | 2 | 3> = {
  "Premier League": 1, "Bundesliga": 1, "Champions League": 1, "Europa League": 1,
  "Eredivisie": 1,
  "La Liga": 2, "Serie A": 2, "Ligue 1": 2,
  "Championship": 3,
};
...
export class CalibrationEngine {
  ...
  private _defaultMetrics(): CalibrationMetrics {
    return { brier:0, recentBrier:0, rps:null, recentRPS:null, clv:0, roi:0, calibFactor:1.0,
      leagueData:{}, bbnParams:{}, driftAlert:false, resolvedCount:0, winRate:0,
      totalPnl:0, totalStaked:0, dynamicRhoParams:{}, clvDecayCalibration:{},
      ruinProb:0, ahAccuracy:{}, zipCoeffs:null } as CalibrationMetrics;
  }
  ...
  async resolveBet(id: string, outcome: string, homeG: number, awayG: number, closeOdds: number): Promise<{ bets: BetRecord[]; metrics: CalibrationMetrics }> {
    ...
    bet.homeGoals = safeNum(homeG); bet.awayGoals = safeNum(awayG);
    bet.closingOdds = safeNum(closeOdds); bet.resolvedAt = new Date().toISOString();
  ...
  calculate(bets: BetRecord[]): CalibrationMetrics {
    ...
    const MIN_CALIB = 10;
    let bSum = 0, cSum = 0, pnl = 0, stk = 0, wins = 0, pSum = 0;
    const lData: Record<string, { pnl:number; stk:number; n:number; bSum:number; wins:number; pSum:number }> = {};
    const goalData: Record<string, { hG:number; aG:number; n:number; zeroZero:number; oneZero:number; zeroOne:number; oneOne:number }> = {};
    ...
    res.forEach(b => {
      ...
      if (b.outcome !== 'push') {
        ...
        const odds = b.odds ?? 1, sa = b.stakeAmt ?? 0;
        const winAmt = b.outcome==='win' ? sa*odds-sa : b.outcome==='half-win' ? (sa/2)*odds-(sa/2) : b.outcome==='loss' ? -sa : b.outcome==='half-loss' ? -(sa/2) : 0;
      ...
      if (b.league) {
        ...
        ld.stk += b.stakeAmt??0; ld.n++; ld.bSum += Math.pow((b.mp??0)-iw,2); ld.wins += iw;
        ld.pSum += b.mp ?? 0;
      }
      if (typeof b.homeGoals === 'number' && typeof b.awayGoals === 'number' && b.league) {
        ...
        gd.hG += b.homeGoals; gd.aG += b.awayGoals; gd.n++;
    ...
    const recentBrier = nonPush.slice(-15).reduce((acc,b) => {
      const a = b.outcome==='win'?1 : b.outcome==='half-win'?0.5 : 0;
      return acc + Math.pow((b.mp??0)-a, 2);
    ...
    rpsBets.forEach(b => {
      ...
      rpsSum += rankedProbabilityScore(b.fp!, actual); rpsN++;
    });
    ...
    const calibFactor = nonPush.length >= MIN_CALIB ? Math.max(0.5, Math.min(1.2, wins / Math.max(0.001, pSum))) : 1.0;
    ...
    const bbnParams: Record<string, { homeAvg: number; awayAvg: number; shrinkage: number; n: number }> = {};
    ...
    const leagueCalibFactors: Record<string, { calibFactor: number; shrinkage: number; n: number }> = {};
    ...
    res.forEach(b => {
      ...
      const score = b.outcome==='win'?1 : b.outcome==='half-win'?0.5 : b.outcome==='half-loss'?0 : b.outcome==='loss'?0 : null;
    ...
    return {
      brier: overallBrier, recentBrier: recentBrier ?? 0,
      rps: overallRPS, recentRPS,
      clv: res.length > 0 ? cSum / res.length : 0,
      roi: stk > 0 ? pnl / stk : 0,
      calibFactor, resolvedCount: res.length,
      leagueData: { ...lData, _leagueCalibFactors: leagueCalibFactors }, bbnParams,
      driftAlert: (overallRPS != null && recentRPS != null) ? (recentRPS > overallRPS + 0.02) : (recentBrier > overallBrier + 0.05),
      winRate: winRateCalc, totalPnl: pnl, totalStaked: stk,
      dynamicRhoParams: {}, clvDecayCalibration: this.backtestCLV(res),
      ruinProb, ahAccuracy: ahAccuracyFlat, zipCoeffs: null,
    };
  }
  ...
  backtestCLV(resolvedBets: BetRecord[]): Record<string, unknown> {
    ...
    Object.keys(byMarket).forEach(mt => {
      ...
      calibration[mt] = { correctionFactor: avgPred > 0 ? clamp(avgActual / avgPred, 0.3, 2.0) : 1.0, avgPredicted: avgPred, avgActual, sampleSize: d.count };
    });
    return calibration;
  }

  async getBets(): Promise<BetRecord[]>          { return this._load(); }
  async getPendingBets(): Promise<BetRecord[]>   { return (await this._load()).filter(b => b.status === 'pending'); }
  async getResolvedBets(): Promise<BetRecord[]>  { return (await this._load()).filter(b => b.status === 'resolved'); }
  async getMetrics(): Promise<CalibrationMetrics>{ return this.calculate(await this._load()); }
}
...
export function significanceAcceptGate(
  ...
  if (n < minN) {
    return {
      accept: false, delta: 0, ciLower: NaN, ciUpper: NaN, n, effectSize: 0,
      reason: `INSUFFICIENT_SAMPLES (n=${n} < minN=${minN})`,
    };
  }

  let baseSum = 0, candSum = 0;
  for (let i = 0; i < n; i++) { baseSum += baseline[i]!; candSum += candidate[i]!; }
  const delta = (candSum - baseSum) / n;
  const effectSize = Math.abs(delta);

  if (effectSize < effectSizeFloor) {
    return {
      accept: false, delta, ciLower: NaN, ciUpper: NaN, n, effectSize,
