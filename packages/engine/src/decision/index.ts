...
function deterministicDecide(eligibleBets: EVMarket[], rationale = 'Deterministic top pick'): DecisionResult {
  if (!eligibleBets.length) {
    return { decision: { primaryPick: 'NO_BET', confidence: 0, rationale: 'No eligible bets', rejectedAndWhy: [] }, replay: null };
  }
  const top = eligibleBets[0]!;
  const pick: PickRef = { market: top.market, side: top.side, odds: top.odds, stake: top.stake };
  return { decision: { primaryPick: pick, confidence: top.modelProb, rationale, rejectedAndWhy: [] }, replay: null };
}

// ── Prompt builder ────────────────────────────────────────────────────────────

function buildPrompt(eligibleBets: EVMarket[], ctx: DecisionContext): string {
  const { fixture, fp, lambdaH, lambdaA, expectedScoreline, regime,
    convergenceTier, convergenceScore, mlAllowed, drawRisk, betTrigger, portfolioCorrelation,
    softContext } = ctx;

  const betLines = eligibleBets.map((m, i) =>
    `${i + 1}. [${m.cat}] ${m.label}  mp=${(m.mp * 100).toFixed(1)}%  odds=${m.odds}  ev=+${(m.ev * 100).toFixed(1)}%  stake=${(m.stake * 100).toFixed(1)}% Kelly`,
  ).join('\n');

  const softLines = (softContext ?? []).map(s =>
    `[${s.kind.toUpperCase()}] ${s.text}`,
  ).join('\n');
  ...
  if (config?.claudeApiKey) {
    try {
      ...
      const raw = await callClaude(
        prompt,
        { config: { claudeApiKey: config.claudeApiKey, geminiApiKey: geminiKey, bankroll: 0 }, requestedAt },
        { model: CLAUDE_DECISION_MODEL, maxTokens: 1024 },
      );
      const replay: DecisionReplay = { prompt, rawResponse: raw, model: CLAUDE_DECISION_MODEL, temperature: 0 };
      const parsed = parseDecisionResponse(raw);
      if (!parsed) {
        console.warn('[decide] Claude parse failure — trying Gemini fallback');
        return await _tryGemini(prompt, geminiKey, requestedAt, eligibleBets, 'Claude parse failure');
      }
      return { decision: parsed, replay };
    } catch (err) {
      const reason = err instanceof Error ? err.message : String(err);
      console.warn(`[decide] Claude failed (${reason}) — trying Gemini fallback`);
...
async function _tryGemini(
  ...
  if (!geminiKey) {
    return deterministicDecide(eligibleBets, `${claudeFailReason} — no Gemini key — deterministic fallback`);
  }

  // ── Tier 2: Gemini 2.5 Pro ────────────────────────────────────────────────
  try {
    const { callGeminiDecision } = await import('@oracle/llm');
    const raw = await callGeminiDecision(
      prompt,
      { config: { claudeApiKey: '', geminiApiKey: geminiKey, bankroll: 0 }, requestedAt },
    );
    const replay: DecisionReplay = { prompt, rawResponse: raw, model: GEMINI_DECISION_MODEL, temperature: 0 };
    const parsed = parseDecisionResponse(raw);
    if (!parsed) {
      console.warn('[decide] Gemini parse failure — falling back to deterministic');
      return { ...deterministicDecide(eligibleBets, 'Gemini parse failure — deterministic fallback'), replay };
    }
    return { decision: parsed, replay };
  } catch (err) {
    const reason = err instanceof Error ? err.message : String(err);
    console.warn(`[decide] Gemini failed (${reason}) — falling back to deterministic`);
    return deterministicDecide(eligibleBets, `Gemini unavailable (${reason}) — deterministic fallback`);
  }
}
