...
export interface ZipCoeffs { b0: number; b1: number; b2: number; }
export interface GoalData {
  n: number; hG: number; aG: number;
  zeroZero?: number; oneZero?: number; zeroOne?: number; oneOne?: number;
}
export interface Weather { wind_mph?: number; rain_mm?: number; }
export interface Referee { cards_per_game?: number; }
/** λ-adjustment result. Carries both `lH/lA` and legacy `lambdaH/lambdaA` aliases (source compat). */
export interface LambdaAdjust { lH: number; lA: number; lambdaH: number; lambdaA: number; penalized: boolean; }
export interface TravelAdjust { lA: number; lambdaA: number; penalized: boolean; }
...
export interface AhBook { qEV: QEVFn; [key: string]: number | Record<string, number | string> | QEVFn; }
export interface MarketBook {
  hw: number; dr: number; aw: number; btts: number; noBtts: number;
  ou: Record<string, number>;
  ah: AhBook;
  dnb_h: number; dnb_a: number; dc_1x: number; dc_x2: number;
  teamH: Record<string, number>; teamA: Record<string, number>;
  asian2: { over: number; under: number };
}
export interface SyntheticScript { title: string; legs: string[]; prob: number; estBookie: number; edge: number; }
export interface VarianceResult { varFlag: boolean; varMultiplier: number; stdDevEst: number; ciBound: number; }
...
export interface RegimeReport {
  ...
  p00: number; lowScoreMass: number; pUnder25: number; expTotal: number; maxSide: number;
  dominantSide: 'home' | 'away' | null;
  pHome: number; pDraw: number; pAway: number;
}
export interface AhCandidate { line: number; side: 'home' | 'away'; pWin: number; pPush: number; pLoss: number; settleProb: number; accuracy?: number; score?: number; }
export interface AhPivotResult {
  pivotApplied: boolean; recommendation: string; side: 'home' | 'away'; line: number;
  settleProb: number; accuracy: number; score: number; rationale: string; allCandidates: AhCandidate[];
}
export interface VigRemoval { home: number; draw: number; away: number; k: number; }
/** Line-movement read from lstmMarketDecoderProxy. */
export interface MarketSignal { velocity: number; rlm: boolean; steam: boolean; sharpCompression: boolean; }
/** clvProjection result (PRD §2.3/§8.3 — CLV is the liquid-market gate). */
export interface ClvProjection { projected: number; survivalProb: number; edgeRetentionFraction: number; decayFactor: number; edgeStrengthFactor?: number; }
export interface RecentMatch { xg?: number; goalsScored?: number; matchdayOffset?: number; }
export interface EloPoint { rating: number; matchday?: number; }
export interface LambdaConsistency { inconsistent: boolean; divergence: number; poissonEstimate?: number; }
export type ScenarioEvent = { type?: string } | string;
export interface RerunBase { bayesian_lH?: number; bayesian_lA?: number; dynamicRho?: number; evMarkets?: Array<{ stake?: number }>; }
export interface RerunResult {
  eventApplied: string;
  lambdaH: { before: number; after: number; delta: number };
  lambdaA: { before: number; after: number; delta: number };
  newMarkets: MarketBook; deltaScore: number; deltaKelly: number;
  newLambdaH: number; newLambdaA: number; interpretation: string;
}
export type Outcome = 'home' | 'draw' | 'away';
export type Forecast = Partial<Record<Outcome, number>>;
export interface RpsRecord { forecast?: Forecast; outcome?: string | Forecast; }
export interface KlResult { kl: number; js: number; hardSignal: boolean; strength: string; bitsAdv: number; maxDivOutcome: string; flag: string | null; }
export interface EfficiencyResult { normProbs: { home: number; draw: number; away: number }; eff: number; flb: string; flag: string | null; }
export interface VarianceRegime { regime: string; factor: number; autocorr: number; l3WinRate?: number; l8WinRate?: number; accel?: number; }
export interface LeeRecovery { multiplier: number; recoveryProb: number; constrained: boolean; flag?: string | null; }
...
export const dixonColesTau = (x: number, y: number, lH: number, lA: number, rho: number): number => {
  if (rho === 0) return 1.0;
...
export const estimateDynamicRho = (goalData: GoalData | null | undefined, baseRho: number): number => {
  ...
  let lo = -0.30, hi = 0.02; const fLoInit = dL(lo), fHi = dL(hi);
  let fLo = fLoInit;
  if (fLo * fHi > 0) return clamp(baseRho, -0.30, 0.02);
  for (let i = 0; i < 60; i++) {
    const mid = (lo + hi) / 2, fMid = dL(mid);
    if (Math.abs(fMid) < 1e-7 || (hi - lo) < 1e-6) return mid;
    if (fLo * fMid < 0) { hi = mid; } else { lo = mid; fLo = fMid; }
  }
  return clamp((lo + hi) / 2, -0.30, 0.02);
};
...
export function sarmanovTau(x: number, y: number, lH: number, lA: number, rho: number, order = 0): number {
...
export function buildMatrix(lH: number, lA: number, rho: number, useZIP = false, zipPi = 0.08, sarmanovOrder = 0): Matrix {
  const mat: number[][] = []; let sum = 0;
  const totalXG = lH + lA;
  ...
  if (sum > 0) for (let i = 0; i < MAX_GOALS; i++) for (let j = 0; j < MAX_GOALS; j++) mat[i][j] = (mat[i][j] || 0) / sum;
  return mat;
}

/** applyEnvironmentalPenalties — line 490. */
export const applyEnvironmentalPenalties = (lH: number, lA: number, weather?: Weather | null, ref?: Referee | null): LambdaAdjust => {
  let mH = 1, mA = 1;
  if ((weather?.wind_mph ?? 0) > 18.5) { mH *= 0.92; mA *= 0.92; }
  if ((weather?.rain_mm ?? 0) > 5.0) { mH *= 0.94; mA *= 0.94; }
  if ((ref?.cards_per_game ?? 0) > 4.5) { mH *= 0.97; mA *= 0.97; }
...
export const applyFatigueDecay = (restH: number, restA: number, lH: number, lA: number): LambdaAdjust => {
  let mH = 1, mA = 1;
  const d = restH - restA;
...
export const applyTravelFriction = (travelKm: number, altitudeM: number, lA: number): TravelAdjust => {
  let modA = 1.0;
...
export const extractMarkets = (mat: Matrix): MarketBook => {
  ...
  const ah: AhBook = { qEV: (pWin, pHalf, pLoss, o) => pWin * (o - 1) + pHalf * 0.5 * (o - 1) - pLoss };
  ...
  const teamH: Record<string, number> = {}; const teamA: Record<string, number> = {};
...
export function detectLowScoringRegime(mat: Matrix, lH: number, lA: number): RegimeReport {
  ...
  const dominantSide: 'home' | 'away' | null = maxSide >= 0.48 ? (m.hw >= m.aw ? 'home' : 'away') : null;
...
export function asianHandicapPivot(mat: Matrix, regime: RegimeReport, leagueAccuracy: Record<string, number> = {}): AhPivotResult {
  const N = mat.length || 14;
  const ahComponents = (line: number, side: 'home' | 'away') => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      for (let j = 0; j < N; j++) {
        ...
        if (Math.abs(adj - 0.25) < 0.01) { pWin += p * 0.5; pPush += p * 0.5; }
        else if (Math.abs(adj + 0.25) < 0.01) { pLoss += p * 0.5; pPush += p * 0.5; }
  ...
  if (regime.dominantSide) {
    ...
    candidates = [ahComponents(0.0, fav), ahComponents(-0.25, fav), ahComponents(0.25, fav), ahComponents(-0.5, fav)];
  } else {
    candidates = [
  ...
  return {
    ...
    side: best.side, line: best.line, settleProb: best.settleProb, accuracy: best.accuracy, score: best.score,
...
export function shinPowerVigRemoval(oddsH: number, oddsD: number, oddsA: number): VigRemoval {
  ...
  return { home: clamp(Math.pow(impH, k), 0.001, 0.999), draw: clamp(Math.pow(impD, k), 0.001, 0.999), away: clamp(Math.pow(impA, k), 0.001, 0.999), k };
}
...
export const lstmMarketDecoderProxy = (modelProb: number, openOdds: number, currentOdds: number, isPopular: boolean): MarketSignal => {
...
export const calculateDHA = (piDiff: number): number => { if (piDiff < -0.8) return 0.85; if (piDiff > 1.2) return 1.15; return 1.0; };
...
export const hurdle = (p: number): number => {
  if (p >= 0.75) return 0.03; if (p >= 0.60) return 0.04; if (p >= 0.40) return 0.06;
  if (p >= 0.20) return 0.09; if (p >= 0.10) return 0.12; return 0.15;
};
...
export const optimizedKelly = (
  edge: number, odds: number, dqs: number, councilPenaltyActive: boolean,
  varMultiplier = 1.0, drawdownPenalty = 1.0, calibFactor = 1.0, base = 0.25, modelProb: number | null = null,
): number => {
  ...
  const mp = modelProb !== null ? modelProb : clamp((edge + 1) / Math.max(1.001, odds), 0.001, 0.999);
  const b = odds - 1;
  if (b <= 0) return 0;
...
export const clvProjection = (edge: number, hoursToKO: number, marketType: string, leagueLiquidity = 1.0): ClvProjection => {
  ...
  const edgeRetentionFraction = clamp(retentionRaw * (0.6 + 0.4 * Math.min(1.0, edgeStrengthFactor)), 0.05, 0.95);
  const logit = 6.0 * (edgeRetentionFraction - 0.50);
...
export const applyTemporalDecay = (recentMatches: RecentMatch[] | null | undefined, baseAvg: number, halfLife = 10): number => {
...
export const drawCalibrationFactor = (poissonDrawProb: number, leagueDrawRate = 0.25): number => {
  ...
  return clamp(ratio, 0.85, 1.20);
};

/** checkLambdaInconsistency — line 1020. Cross-validates λ vs O/U 2.5 implied; flags >5% divergence. */
export const checkLambdaInconsistency = (lH: number, lA: number, ou25ImpliedProb: number): LambdaConsistency => {
...
export const isSteamChaser = (sharpCompression: boolean, edge: number): boolean => sharpCompression && edge < 0.05;

/** rerunWithOverride — line 1042. Scenario branching: re-price with an injected event (B15). */
export function rerunWithOverride(event: ScenarioEvent | null | undefined, baseResult: RerunBase | null | undefined): RerunResult | null {
  ...
  for (const [key, delta] of Object.entries(adjustments)) {
    if (eventType.includes(key.replace(/_/g, ' ')) || eventType.includes(key)) { adj = delta; break; }
  }
  ...
  const oldXG = lH + lA, newXG = newLH + newLA, xgDelta = newXG - oldXG;
  ...
  return {
    ...
    newMarkets, deltaScore, deltaKelly, newLambdaH: newLH, newLambdaA: newLA,
...
export function rankedProbabilityScore(forecast: Forecast | null | undefined, outcome: string | Forecast | null | undefined): number {
...
export function meanRPS(records: RpsRecord[] | null | undefined): { rps: number | null; n: number } {
  let sum = 0, n = 0;
...
export function normalizedEfficiency(oddsH: number, oddsD: number, oddsA: number, mH: number, mD: number, mA: number): EfficiencyResult | null {
...
export function leeRecoveryConstraint(drawdown: number, betsRemaining = 50, _targetRecovery = 1.0): LeeRecovery {
  ...
  const multiplier = recoveryProb >= 0.70 ? 1.0 : recoveryProb >= 0.50 ? 0.85 : recoveryProb >= 0.30 ? 0.65 : 0.50;
  return {
    multiplier: parseFloat(multiplier.toFixed(3)), recoveryProb: parseFloat(recoveryProb.toFixed(3)), constrained: multiplier < 1.0,
...
export const DEFAULT_BIVARIATE_LAMBDA3 = 0.10;
...
export function buildBivariateMatrix(lH: number, lA: number, lambda3 = DEFAULT_BIVARIATE_LAMBDA3): Matrix {
...
export function skellamPMF(k: number, l1: number, l2: number): number {
  ...
  return Math.exp(-(s1 + s2) + (k / 2) * Math.log(s1 / s2))
