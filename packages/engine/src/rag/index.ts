...
const LEAGUE_NAMES = [
  ...
  'Ligue 1', 'Eredivisie', 'Champions League', 'Default',
];
const MAX_STORE = 200;
...
export class RAGSystem {
  ...
  createEmbedding(fd: Record<string, unknown>): number[] {
    ...
    const leagueHash = (leagueIdx >= 0 ? leagueIdx : LEAGUE_NAMES.length - 1) / Math.max(1, LEAGUE_NAMES.length - 1);
  ...
  async addToStore(fd: Record<string, unknown>, result: Record<string, unknown>): Promise<void> {
    ...
    const cleanEmb = rawEmb.map(v => (isNaN(v) || !isFinite(v)) ? 0 : v);
    ...
    this._store.push({
      ...
      away: String(fd['away'] ?? ''),
      league,
      competitionType: String(fd['competitionType'] ?? (league.toLowerCase().includes('cup') ? 'cup' : 'league')),
  ...
  formatAnalogues(similar: RAGEntry[]): string {
    ...
    return similar.slice(0, 3).map((s, i) => {
      const res = s.result;
      const topEv = (res?.['evMarkets'] as Array<Record<string, number>> | undefined)?.[0]?.['ev'] ?? 0;
      const topBanker = (res?.['debate'] as Record<string, unknown> | undefined)?.['topBankerBet'] ?? 'N/A';
  ...
  getStore(): RAGEntry[] { return [...this._store]; }
  ...
  auditStoreBenford(): string | null {
    ...
    if (mad > 0.015) return `[BENFORD_ANOMALY_DATA_SOURCE] RAG store lambda Benford MAD=${mad.toFixed(4)} (threshold 0.015). ${lambdas.length} values.`;
    if (mad > 0.006) return `[BENFORD_ACCEPTABLE_DATA_SOURCE] RAG store lambda MAD=${mad.toFixed(4)} — within acceptable range.`;
    return null;
  }
}
...
export class PostmortemRegistry {
  ...
  private _buildEmbedding(e: Omit<PostmortemEntry, 'embedding' | 'addedAt' | 'similarity'>): number[] {
  ...
  check(queryEntry: Omit<PostmortemEntry, 'embedding' | 'addedAt' | 'similarity'>): PostmortemEntry[] {
  ...
  formatWarning(matches: PostmortemEntry[]): string {
    ...
    matches.slice(0, 3).forEach((m, i) => {
      lines.push(`[${i+1}] ${m.homeTeam} vs ${m.awayTeam} (${m.date}) — ${m.rootCause} — Market: ${m.marketPicked} -> LOSS (sim:${((m.similarity ?? 0)*100).toFixed(0)}%)`);
    });
    lines.push('ACTION: Verify these failure conditions are NOT present before recommending.');
    return lines.join('\n');
  }

  getAll(): PostmortemEntry[] { return [...this._entries]; }
...
postmortemRegistry.add({
  fixtureId: 'GAL_LIV_20260310', date: '2026-03-10',
  homeTeam: 'Galatasaray', awayTeam: 'Liverpool',
  marketPicked: 'BTTS Yes', marketResult: 'loss',
...
postmortemRegistry.add({
  fixtureId: 'NEW_BAR_20260310', date: '2026-03-10',
  homeTeam: 'Newcastle', awayTeam: 'Barcelona',
  marketPicked: 'Over 3', marketResult: 'loss',
...
postmortemRegistry.add({
  fixtureId: 'POR_SWA_20260310', date: '2026-03-10',
  homeTeam: 'Portsmouth', awayTeam: 'Swansea',
  marketPicked: 'Under 2.5', marketResult: 'loss',
...
postmortemRegistry.add({
  fixtureId: 'STO_IPS_20260310', date: '2026-03-10',
  homeTeam: 'Stoke City', awayTeam: 'Ipswich Town',
  marketPicked: 'Away ML', marketResult: 'loss',
