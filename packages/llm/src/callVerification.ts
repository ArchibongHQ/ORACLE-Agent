...
function parseCvlResponse(text: string): { status: CvlStatus; rationale: string; override?: string } {
  try {
    const cleaned = text.replace(/```(?:json)?\s*/gi, '').replace(/```\s*/g, '').trim();
    const start = cleaned.indexOf('{');
    const end = cleaned.lastIndexOf('}');
    if (start === -1 || end === -1) return { status: 'APPROVED', rationale: 'parse error — defaulting APPROVED' };
...
export async function callVerification(
  ...
  if (!ctx.config.claudeApiKey) {
    return { status: 'SKIPPED', stamp: new Date().toISOString(), rationale: 'no Claude key', model: 'none' };
  }
