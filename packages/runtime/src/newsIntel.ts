...
async function writeCache(home: string, away: string, data: NewsCache): Promise<void> {
  ...
  await writeFile(join(NEWS_DIR, `${slug(home, away)}.json`), JSON.stringify(data, null, 2), 'utf8');
}
...
export async function enrichWithNewsIntel(
  ...
  console.log(`[news] Enriching ${eligible.length} fixtures with Perplexity intelligence`);
  ...
  for (const { job, idx } of eligible) {
    try {
      ...
      if (!cached) {
        ...
        const intel = await fetchNewsIntelligence(job.home, job.away, job.league, job.kickoff, apiKey);
        apiCalls++;
        if (intel) {
      ...
      console.warn(`[news] ${job.home} vs ${job.away}: ${msg}`);
    }
  }

  if (filled > 0) {
    console.log(`[news] Enriched ${filled}/${eligible.length} fixtures (${apiCalls} API calls, ${eligible.length - apiCalls} cache hits)`);
  }
