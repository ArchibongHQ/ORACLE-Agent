...
import {
  runBatch, ANALYSIS_SCHEMA_VERSION, RUN_MANIFEST_SCHEMA_VERSION,
} from '@oracle/engine';
import type {
  OracleConfig, AnalysisRecord, RunManifest, FixtureOutcome, PickRef,
  FixtureJob, BatchResult, BatchOptions,
} from '@oracle/engine';
...
export async function runAnalysis(
  ...
  const records: AnalysisRecord[] = batch.jobs.flatMap(j => {
    ...
    return [{
      ...
      regime:               String((r['lowScoreRegime'] as Record<string, unknown> | undefined)?.regime ?? 'STANDARD'),
  ...
  if (persist && records.length > 0) {
    await storage.upsertBulk(STORAGE_KEYS.analysisRecords, records as unknown as Record<string, unknown>[], 'analysisId');
  }
  ...
  const fixtures: FixtureOutcome[] = batch.jobs.map(j => {
    if (j.status === 'error') {
      return {
        fixtureId: j.fixtureId, home: j.home, away: j.away, league: j.league, kickoff: j.kickoff,
        status: 'error', pick: null, confidence: null,
        errorCode: j.errorCode, errorMessage: j.reason, stakePct: null,
      } satisfies FixtureOutcome;
    }
    const pick = j.decision.primaryPick;
    return {
      fixtureId: j.fixtureId, home: j.home, away: j.away, league: j.league, kickoff: j.kickoff,
      status: 'ok', pick, confidence: j.decision.confidence,
...
export async function resolveDay(
  ...
  const { resolved, unmatched } = await resolveRecords(dayRecords, keys.footballDataApiKey, keys.oddsApiKey);
  if (resolved.length) {
    ...
    try {
      ...
      const losses = resolved
        ...
        .map(r => {
          ...
          const won = r.actualResult === (picked.toLowerCase().includes('away') ? 'away' : picked.toLowerCase().includes('draw') ? 'draw' : 'home');
          return won ? null : {
      ...
      if (losses.length) {
        ...
        for (const r of synthResults) {
          if (r.synthesizedRule) {
            console.log(`[postmortem] ${r.homeTeam} vs ${r.awayTeam}: ${r.synthesizedRule}`);
          }
        }
      }
    } catch { /* non-fatal */ }
