...
import type { RunState } from '@oracle/engine';
...
function extractOdds(bk: OddsBookmaker, homeTeam: string, awayTeam: string): {
...
async function fetchSportOdds(
  ...
  if (remaining) console.log(`[odds-api] ${sportKey}: ${remaining} requests remaining`);
...
async function mergeCache(jobs: FixtureJob[]): Promise<void> {
  ...
  for (const job of jobs) {
    const slug = `${job.home}_vs_${job.away}`.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '');
...
function findUnmatched(scrapedJobs: FixtureJob[], oddsJobs: FixtureJob[]): FixtureJob[] {
  return scrapedJobs.filter(s => {
    ...
    return !oddsJobs.some(o =>
      o.kickoff.slice(0, 10) === sD &&
      namesMatch(o.home, s.home) && namesMatch(o.away, s.away)
    ) && sH.length > 0 && sA.length > 0;
  });
}
...
async function geminiOddsGapFill(
  ...
  console.log(`[odds] Gemini gap-fill: attempting odds for ${unmatched.length} unmatched fixtures`);
  ...
  if (successCount > 0) {
    console.log(`[odds] Gemini gap-fill: ${successCount}/${unmatched.length} fixtures filled`);
  }
...
async function fetchWebSearchOdds(
  ...
  console.log(`[oracle] Attempting web search fallback for ${jobs.length} fixtures`);
  ...
  try {
    ...
    const result = spawnSync('python', [
      join(ROOT, 'tools/scrape_live_odds.py'),
      '--fixtures', tmpPath,
      '--quiet',
    ], {
      encoding: 'utf8',
    ...
    if (result.error) {
      console.warn(`[oracle] Web search scraper failed:`, result.error);
      return jobs;
    }

    if (result.status !== 0) {
      console.warn(`[oracle] Web search scraper exited with code ${result.status}`);
      return jobs;
    }
    ...
    if (successCount > 0) {
      console.log(`[oracle] Web search fallback succeeded for ${successCount}/${jobs.length} fixtures`);
    }

    return jobs;
  } catch (err) {
    console.warn(`[oracle] Web search fallback error:`, err);
    return jobs;
  }
}
...
export async function fetchTodaysFixtures(
  ...
  if (errors.length) console.warn('[odds] Errors during fetch:', errors.join('; '));

  if (oddsJobs.length > 0) {
    // Merge into today.txt without overwriting scraped fixtures
    await mergeCache(oddsJobs);
    console.log(`[odds] Fetched ${oddsJobs.length} fixtures from Odds API`);
  ...
  if (quotaExhausted && enableWebSearchFallback) {
    ...
    if (cached.length > 0) {
      ...
      return { jobs: enrichedJobs, source: 'web_search_consensus', quality: 'degraded', fetchedAt: new Date().toISOString() };
    }
  }
