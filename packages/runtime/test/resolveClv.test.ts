...
describe('computeRealisedClv', () => {
  const frozen = { home: 2.00, draw: 3.40, away: 4.50 };

  it('returns null when frozenOdds side is missing', () => {
    expect(computeRealisedClv({}, { home: 2.10, draw: 3.30, away: 4.20 }, 'Home')).toBeNull();
  });

  it('returns null when analysis odds ≤ 1', () => {
    expect(computeRealisedClv({ home: 0.9 }, { home: 2.00, draw: 3.40, away: 4.50 }, 'Home')).toBeNull();
  });

  it('returns null when closing odds ≤ 1', () => {
    expect(computeRealisedClv(frozen, { home: 0.5, draw: 3.40, away: 4.50 }, 'Home')).toBeNull();
  });
  ...
  it('computes positive CLV when market shortens (closing IP > analysis IP)', () => {
    ...
    const clv = computeRealisedClv(frozen, { home: 1.80, draw: 3.50, away: 4.60 }, 'Home');
    expect(clv).not.toBeNull();
    expect(clv!).toBeGreaterThan(0);
    expect(clv!).toBeCloseTo(1 / 1.80 - 1 / 2.00, 5);
  });
  ...
  it('computes negative CLV when market drifts (closing IP < analysis IP)', () => {
    ...
    const clv = computeRealisedClv(frozen, { home: 2.20, draw: 3.30, away: 4.40 }, 'Home');
    expect(clv).not.toBeNull();
    expect(clv!).toBeLessThan(0);
  });

  it('uses home side as fallback when topPickLabel is null', () => {
    const clvNull  = computeRealisedClv(frozen, { home: 1.90, draw: 3.40, away: 4.50 }, null);
    const clvHome  = computeRealisedClv(frozen, { home: 1.90, draw: 3.40, away: 4.50 }, 'Home');
    expect(clvNull).toEqual(clvHome);
  });

  it('uses draw side when topPickLabel is Draw', () => {
    const clv = computeRealisedClv(frozen, { home: 2.00, draw: 3.10, away: 4.50 }, 'Draw');
    expect(clv).not.toBeNull();
    // closing draw IP (1/3.10) vs analysis draw IP (1/3.40)
    expect(clv!).toBeCloseTo(1 / 3.10 - 1 / 3.40, 5);
  });

  it('uses away side when topPickLabel is Away', () => {
    const clv = computeRealisedClv(frozen, { home: 2.00, draw: 3.40, away: 4.20 }, 'Away');
    expect(clv).not.toBeNull();
    expect(clv!).toBeCloseTo(1 / 4.20 - 1 / 4.50, 5);
  });

  it('result is rounded to 6 decimal places', () => {
    const clv = computeRealisedClv(frozen, { home: 1.95, draw: 3.40, away: 4.50 }, 'Home');
    expect(clv).not.toBeNull();
...
describe('resolveRecords CLV integration', () => {
  ...
  it('resolveRecord produces realisedCLV when Odds API returns matching game', async () => {
    ...
    const record = {
      ...
      evMarkets:            [],
      llmPick:              null,
      deterministicTopPick: { cat: '1X2', label: 'Home', market: '1X2', mp: 0.50, modelProb: 0.50, ip: 0.526, rawEdge: -0.026, ev: -0.026, odds: 1.90, stake: 0.01, stakeAmt: 10, rankingScore: 0.5, varianceMod: 1 },
      frozenOddsAtAnalysis: { home: 1.90, draw: 3.40, away: 4.50 },
  ...
  it('resolveRecord leaves realisedCLV null when Odds API returns no match', async () => {
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
      ...
      const body = url.includes('the-odds-api') ? [] : {
        matches: [{
          id: 2, utcDate: '2026-06-01T15:00:00Z', status: 'FINISHED',
    ...
    const record = {
      fixtureId: 'test-2', home: 'Arsenal', away: 'Chelsea',
      league: 'Premier League', kickoff: '2026-06-01T15:00:00Z',
      lambdaH: 1.5, lambdaA: 1.2,
      probabilities: { home: 0.45, draw: 0.30, away: 0.25 },
      regime: 'STANDARD', rankingMode: 'CONFIDENCE_WEIGHTED' as const,
      evMarkets: [], llmPick: null, deterministicTopPick: null,
      frozenOddsAtAnalysis: { home: 1.90, draw: 3.40, away: 4.50 },
      liquidityTag: 'CLV_ELIGIBLE' as const,
      analysedAt: '2026-06-01T09:00:00Z',
    };
  ...
  it('resolveRecord leaves realisedCLV null for CALIBRATION_ONLY records', async () => {
    vi.spyOn(globalThis, 'fetch').mockImplementation(async () =>
      new Response(JSON.stringify({
        matches: [{
          id: 3, utcDate: '2026-06-01T15:00:00Z', status: 'FINISHED',
          homeTeam: { name: 'Burnley' }, awayTeam: { name: 'QPR' },
    ...
    const record = {
      fixtureId: 'test-3', home: 'Burnley', away: 'QPR',
      league: 'Championship', kickoff: '2026-06-01T15:00:00Z',
      lambdaH: 1.6, lambdaA: 1.1,
      probabilities: { home: 0.55, draw: 0.25, away: 0.20 },
      regime: 'STANDARD', rankingMode: 'CONFIDENCE_WEIGHTED' as const,
      evMarkets: [], llmPick: null, deterministicTopPick: null,
      frozenOddsAtAnalysis: { home: 1.70, draw: 3.50, away: 5.00 },
