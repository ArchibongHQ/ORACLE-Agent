...
import {
  summarizeBatch, formatSummaryText, buildNotifiers, notifyAll,
  TelegramNotifier, SlackNotifier, EmailNotifier, OpenClawNotifier,
} from '../src/index.js';
...
function fakeBatch(overrides: Partial<Record<string, unknown>> = {}) {
  return {
    date: '2026-06-05', rankingMode: 'CONFIDENCE_WEIGHTED', runId: 'r1', calibrationSnapshotId: 'c1',
    completedCount: 2, actionableCount: 1, errorCount: 0, totalRecommendedStakePct: 3,
    cost: { estimatedUsd: 0, ceilingUsd: null, halted: false }, errors: [],
    jobs: [
      {
        status: 'ok', home: 'Arsenal', away: 'Chelsea', league: 'Premier League', kickoff: '2026-06-05T15:00:00Z',
        decision: { primaryPick: { market: 'Goals O/U', side: 'Over 2.5', odds: 2.1, stake: 0.03 }, confidence: 0.78 },
      },
      {
        status: 'ok', home: 'A', away: 'B', league: 'L', kickoff: 'k',
...
const sampleSummary: BatchSummary = {
  date: '2026-06-05', analysed: 2, actionableCount: 1, errors: 0,
  actionable: [{ home: 'Arsenal', away: 'Chelsea', league: 'Premier League', kickoff: 'k', market: 'Goals O/U', side: 'Over 2.5', odds: 2.1, stakePct: 3, confidence: 0.78 }],
...
describe('buildNotifiers', () => {
  ...
  it('all channels configured → four notifiers including openclaw', () => {
    const ns = buildNotifiers({
      ...
      MAIL_API_KEY: 'k', MAIL_FROM: 'a@b.c', MAIL_TO: 'd@e.f',
      OPENCLAW_GATEWAY_URL: 'http://127.0.0.1:18789', OPENCLAW_TOKEN: 'oc-tok',
    });
...
describe('adapters POST correctly', () => {
  ...
  beforeEach(() => { fetchMock.mockReset(); vi.stubGlobal('fetch', fetchMock); fetchMock.mockResolvedValue({ ok: true, status: 200 }); });
  ...
  it('throws on non-ok response', async () => {
    ...
    await expect(new SlackNotifier('http://hook').notify(sampleSummary)).rejects.toThrow(/HTTP 500/);
  });

  it('OpenClaw POSTs to /v1/responses with correct headers and body', async () => {
    await new OpenClawNotifier({ gatewayUrl: 'http://127.0.0.1:18789', token: 'oc-tok', agentId: 'main' }).notify(sampleSummary);
  ...
  it('OpenClaw defaults agentId to main', async () => {
    await new OpenClawNotifier({ gatewayUrl: 'http://127.0.0.1:18789', token: 'tok' }).notify(sampleSummary);
  ...
  it('OpenClaw throws on non-ok Gateway response', async () => {
    ...
    await expect(
      new OpenClawNotifier({ gatewayUrl: 'http://127.0.0.1:18789', token: 'bad' }).notify(sampleSummary),
